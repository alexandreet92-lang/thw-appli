...
<!-- SECRETS RETIRÉS — ne jamais committer de token / clé secrète / identifiant ici.
     Ce fichier est versionné dans le repo. Mettre les secrets Meta / Instagram (et tout
     autre secret) dans .env.local en local et dans les variables d'environnement Vercel.
     ⚠️ Les valeurs qui figuraient ici (access token Meta, client secret, IG business id)
     ont été exposées et RESTENT dans l'historique git : elles doivent être RÉVOQUÉES puis
     RÉGÉNÉRÉES côté Meta (developers.facebook.com) — c'est la seule façon de les neutraliser. -->
