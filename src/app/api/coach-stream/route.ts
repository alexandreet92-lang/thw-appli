...
(Les deux parties doivent être DIFFÉRENTES : l'oral discute, l'écrit schématise. Cette consigne annule toute consigne de format/markdown donnée plus haut pour la partie orale.)`
  ...
  const readable = new ReadableStream({
    async start(controller) {
      ...
      // Conteneur d'exécution serveur (web_search 2026 tourne dans un sandbox) :
      // dès qu'une réponse en crée un, il FAUT le renvoyer aux tours suivants,
      // sinon l'API renvoie « container_id is required when there are pending tool
      // uses generated by code execution with tools » (400) sur les pause_turn.
      ...
      try {
        for (let step = 0; step < MAX_STEPS; step++) {
          ...
          if (respContainer?.id) containerId = respContainer.id
