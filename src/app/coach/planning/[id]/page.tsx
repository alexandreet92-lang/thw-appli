...
export default function CoachAthletePlanningPage() {
  ...
  const analyze = () => window.dispatchEvent(new CustomEvent('thw:open-coach', { detail: { prompt: `Analyse ${athlete?.name ?? 'cet athlète'} : sa charge d'entraînement, sa récupération et son plan de la semaine, et propose des ajustements concrets.` } }))
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 40, background: 'color-mix(in srgb, var(--bg) 92%, transparent)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', borderBottom: '1px solid var(--border)', paddingTop: 'calc(env(safe-area-inset-top, 0px) + 58px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', padding: '0 clamp(16px,4vw,40px) 12px', maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 11, minWidth: 0, flex: '1 1 230px' }}>
            <Link href="/coach/planning" aria-label="Retour aux athlètes" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 34, height: 34, borderRadius: 10, color: 'var(--text-mid)', flexShrink: 0, textDecoration: 'none', border: '1px solid var(--border)', background: 'var(--bg-card)' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6" /></svg>
            </Link>
            <Avatar url={athlete?.avatar ?? null} name={athlete?.name ?? null} size={40} />
            <div style={{ minWidth: 0 }}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: 16.5, fontWeight: 700, color: 'var(--text)', lineHeight: 1.15, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{athlete?.name ?? 'Athlète'}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 'auto', flexShrink: 0 }}>
            ...
            <button onClick={() => athlete && openCoachMessage({ athleteId, name: athlete.name, avatar: athlete.avatar })} aria-label="Message" style={{ ...iconBtn, padding: '0 10px' }}>
            ...
            <Link href={`/coach/athlete/${athleteId}`} aria-label="Fiche 360°" style={{ ...iconBtn, padding: '0 10px' }}>
