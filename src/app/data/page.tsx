...
const LOAD_WEEKS = [
  {w:'S7', ctl:72,atl:68, tsb:4 },{w:'S8', ctl:76,atl:82, tsb:-6},
  {w:'S9', ctl:78,atl:88, tsb:-10},{w:'S10',ctl:80,atl:75,tsb:5 },
  {w:'S11',ctl:82,atl:90, tsb:-8},{w:'S12',ctl:84,atl:91,tsb:-7},
]
...
interface FoodEntry { id:string; name:string; cal:number; p:number; g:number; l:number }
...
const INITIAL_MEALS: MealSection[] = [
  {type:'breakfast',label:'Petit-déjeuner',emoji:'☀️',entries:[
    ...
    {id:'f2',name:'Café',cal:5,p:0,g:1,l:0},
  ]},
...
const INITIAL_CHAT: ChatMessage[] = [{
  id:'0',role:'ai',
  content:'👋 Bonjour ! Je suis ton assistant nutrition IA.\n\nUtilise les sections ci-dessus pour enregistrer tes repas, ou pose-moi une question sur ta nutrition.',
  timestamp:'09:00',
}]
...
const SAVED_MEALS_INIT: SavedMeal[] = [
  {id:'sm1',name:'Petit-dej endurance',totalCal:520,entries:[
    ...
    {id:'se3',name:'Jus orange',cal:100,p:1,g:23,l:0},
  ]},
  {id:'sm2',name:'Récup post-séance',totalCal:380,entries:[
    {id:'se4',name:'Shake whey + lait 250ml',cal:250,p:28,g:18,l:6},
    {id:'se5',name:'Banane',cal:90,p:1,g:23,l:0},
  ]},
]

const AI_MEAL = [
  (n:string,p:number,g:number,l:number,c:number) => `✅ ${n} analysé (~${c} kcal)\n\nP: ${p}g · G: ${g}g · L: ${l}g\n\n💡 ${p>=25?'Bon apport protéique ✓':'Protéines un peu faibles.'}. ${g>=50?'Glucides suffisants ✓':'Glucides bas pour une séance intense.'}`,
  (n:string,p:number,g:number,l:number,c:number) => `📊 ${n} (~${c} kcal)\n\nP:${p}g · G:${g}g · L:${l}g\n\nRepas adapté. ${g>60?'Glucides bien présents pour l\'énergie.':'Pense à augmenter les glucides si séance intense.'}`,
]
const AI_CHAT = [
...
function totalMacros(meals:MealSection[]) {
  ...
  return {cal:all.reduce((s,e)=>s+e.cal,0),p:all.reduce((s,e)=>s+e.p,0),g:all.reduce((s,e)=>s+e.g,0),l:all.reduce((s,e)=>s+e.l,0)}
}

function parseFood(text:string):FoodEntry {
  const w=text.toLowerCase()
  let cal=0,p=0,g=0,l=0
  if(w.includes('œuf')||w.includes('oeuf'))  {cal+=80;p+=7;g+=0;l+=5}
  if(w.includes('pain'))   {cal+=80;p+=3;g+=16;l+=1}
  if(w.includes('riz'))    {cal+=130;p+=3;g+=28;l+=0}
  if(w.includes('poulet')) {cal+=165;p+=31;g+=0;l+=4}
  if(w.includes('pâtes')||w.includes('pasta')){cal+=200;p+=7;g+=40;l+=1}
  if(w.includes('banane')) {cal+=90;p+=1;g+=23;l+=0}
  if(w.includes('lait'))   {cal+=60;p+=3;g+=5;l+=3}
  if(w.includes('yaourt')) {cal+=100;p+=10;g+=7;l+=3}
  if(w.includes('whey')||w.includes('protéine')){cal+=130;p+=25;g+=5;l+=2}
  if(w.includes('café'))   {cal+=5;p+=0;g+=1;l+=0}
  if(w.includes('fromage')){cal+=110;p+=8;g+=0;l+=9}
  if(w.includes('saumon')) {cal+=180;p+=20;g+=0;l+=11}
  if(w.includes('avocat')) {cal+=160;p+=2;g+=6;l+=15}
  if(cal===0){cal=Math.floor(Math.random()*300+150);p=Math.floor(Math.random()*25+8);g=Math.floor(Math.random()*40+15);l=Math.floor(Math.random()*12+4)}
  return {id:uid(),name:text,cal,p,g,l}
}

function calcBMR(w:number,h:number,a:number,s:'m'|'f'):number {
  return s==='m'?10*w+6.25*h-5*a+5:10*w+6.25*h-5*a-161
}
...
function generatePlan(q:QuestionnaireData):NutriPlan {
  ...
  return {
    calories:Math.round(base),protein,carbs,fat,
    byDay:{
      low: {cal:Math.round(base*0.85),p:protein,g:Math.round(carbs*0.70),l:fat},
      mid: {cal:Math.round(base),     p:protein,g:carbs,                 l:fat},
      hard:{cal:Math.round(base*1.15),p:protein,g:Math.round(carbs*1.25),l:fat},
    }
  }
}
      ...
      {opts.map(([id,l])=>(
        <button key={id} onClick={()=>onChange(id)}
          style={{padding:'3px 9px',borderRadius:6,border:'1px solid',fontSize:10,cursor:'pointer',
            ...
            color:value===id?'#00c8e0':'var(--text-dim)',
    ...
    <div style={{position:'relative',width:size,height:size,flexShrink:0}}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{transform:'rotate(-90deg)'}}>
        ...
        {slices.filter(s=>s.pct>0).map((s,i)=>{
          const dash=(s.pct/100)*c,off=-cum*c/100; cum+=s.pct
          return <circle key={i} cx={size/2} cy={size/2} r={r} fill="none" stroke={s.color} strokeWidth={stroke} strokeLinecap="butt" strokeDasharray={`${dash} ${c}`} strokeDashoffset={off} opacity={0.85}/>
        })}
      </svg>
          ...
          <div style={{minWidth:300,display:'flex',gap:6,alignItems:'flex-end',height:80}}>
            {LOAD_WEEKS.map((w,i)=>(
              <div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:3}}>
                <div style={{width:'100%',display:'flex',flexDirection:'column',gap:2,alignItems:'stretch'}}>
      ...
      <div style={{background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:16,padding:20,boxShadow:'var(--shadow-card)'}}>
        ...
        <div style={{display:'flex',alignItems:'flex-end',gap:3,height:52}}>
          {SLEEP_DATA.slice(-14).map((v,i)=>{
            const pct=v/9,c=v>=7.5?'#22c55e':v>=6.5?'#ffb340':'#ef4444'
            return <div key={i} style={{flex:1,height:`${pct*52}px`,minHeight:2,borderRadius:'3px 3px 0 0',background:`linear-gradient(180deg,${c}bb,${c}44)`}}/>
          })}
        </div>
        <div style={{display:'flex',justifyContent:'space-between',marginTop:4,fontSize:10,color:'var(--text-dim)'}}>
...
function NutritionTab() {
  ...
  const macroSlices=[
    {pct:totals.p*4/macroTotal*100,color:'#3b82f6'},
    {pct:totals.g*4/macroTotal*100,color:'#22c55e'},
    {pct:totals.l*9/macroTotal*100,color:'#f97316'},
  ]
  ...
  function sendChat() {
    ...
    setTimeout(()=>{
      const am:ChatMessage={id:uid(),role:'ai',content:AI_CHAT[Math.floor(Math.random()*AI_CHAT.length)],timestamp:nowTime()}
  ...
  function logMeal(mealType:MealType) {
    ...
    const entry=parseFood(mealInput)
    setMeals(p=>p.map(m=>m.type===mealType?{...m,entries:[...m.entries,entry]}:m))
    const rn=AI_MEAL[Math.floor(Math.random()*AI_MEAL.length)]
    const secLabel=meals.find(m=>m.type===mealType)?.label||''
    const am:ChatMessage={id:uid(),role:'ai',content:`📝 Ajouté à ${secLabel}\n\n`+rn(mealInput,entry.p,entry.g,entry.l,entry.cal),timestamp:nowTime()}
    setChat(p=>[...p,am])
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          <div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {[
                {v:'low',     l:'Faible',  d:'Travail de bureau'},
                {v:'moderate',l:'Modérée', d:'Actif au quotidien'},
                {v:'high',    l:'Élevée',  d:'Travail physique'},
              ].map(o=>(
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
          ...
          <div key={section.type} style={{background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:14,overflow:'hidden',boxShadow:'var(--shadow-card)'}}>
            ...
            {isLog&&(
              <div style={{padding:'12px 16px',borderTop:'1px solid var(--border)',background:'var(--bg-card2)'}}>
                <p style={{fontSize:11,color:'var(--text-dim)',marginBottom:6}}>Décris ce que tu as mangé — l'IA calcule les macros</p>
                <div style={{display:'flex',gap:8}}>
                  <input value={mealInput} onChange={e=>setMealInput(e.target.value)} onKeyDown={e=>{ if(e.key==='Enter') logMeal(section.type) }}
                    placeholder="Ex: 150g riz, filet poulet, brocolis…"
                  ...
                  <button onClick={()=>logMeal(section.type)}
                    style={{padding:'8px 16px',borderRadius:9,background:'linear-gradient(135deg,#00c8e0,#5b6fff)',border:'none',color:'#fff',fontSize:13,fontWeight:600,cursor:'pointer'}}>
                    → Ajouter
                  </button>
                </div>
                {savedMeals.length>0&&(
      ...
      <div style={{background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:16,overflow:'hidden',boxShadow:'var(--shadow-card)'}}>
        <div style={{padding:'13px 16px',borderBottom:'1px solid var(--border)'}}>
          ...
          <p style={{fontSize:11,color:'var(--text-dim)',margin:'2px 0 0'}}>Pose une question sur ta nutrition — ne pas confondre avec l'ajout de repas ci-dessus</p>
        </div>
        ...
        <div style={{padding:'10px 14px',borderTop:'1px solid var(--border)',background:'var(--bg-card2)'}}>
          <div style={{display:'flex',gap:8,alignItems:'center'}}>
            ...
            <div style={{position:'relative'}}>
              ...
              {plusOpen&&(
                <div style={{position:'absolute',bottom:'calc(100% + 8px)',left:0,background:'var(--bg-card)',border:'1px solid var(--border-mid)',borderRadius:12,padding:6,boxShadow:'var(--shadow)',zIndex:50,minWidth:200}}>
                  {[
            ...
            {planStep===null&&(
              <div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:7,marginBottom:16}}>
                  {['Calcul du métabolisme de base (formule Mifflin)','Ajustement selon ta charge sportive hebdomadaire','Répartition optimale des macros (P/G/L)','Adaptation par type de journée — Low / Mid / Hard'].map((t,i)=>(
            ...
            {planStep==='result'&&nutPlan&&(
              <div>
                <div style={{padding:'14px 16px',borderRadius:12,background:'rgba(0,200,224,0.07)',border:'1px solid rgba(0,200,224,0.15)',marginBottom:14}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:8}}>
                    {[
                      {l:'Calories', v:`${nutPlan.calories} kcal`,c:'#ffb340'},
                      {l:'Protéines',v:`${nutPlan.protein}g`,c:'#3b82f6'},
                      {l:'Glucides', v:`${nutPlan.carbs}g`,  c:'#22c55e'},
                      {l:'Lipides',  v:`${nutPlan.fat}g`,    c:'#f97316'},
                    ].map(x=>(
                ...
                {[
                  {l:'Repos / Low', d:nutPlan.byDay.low,  c:'#22c55e'},
                  {l:'Mid',         d:nutPlan.byDay.mid,  c:'#ffb340'},
                  {l:'Hard',        d:nutPlan.byDay.hard, c:'#ef4444'},
                ].map(dt=>(
                  ...
                  <div style={{display:'flex',gap:5,marginTop:10,flexWrap:'wrap' as const}}>
                    {meals.map(m=>(
                      <button key={m.type} onClick={()=>useSavedMeal(sm,m.type)}
                        style={{flex:1,minWidth:60,padding:'5px 4px',borderRadius:7,border:'1px solid var(--border)',background:'var(--bg-card)',color:'var(--text-dim)',fontSize:10,cursor:'pointer'}}>
...
export default function DataPage() {
  ...
  const TABS:[MainTab,string,string,string,string][]=[
        ...
        {TABS.map(([id,,label,color,bg])=>(
          <button key={id} onClick={()=>setTab(id)}
            style={{flex:1,minWidth:100,padding:'11px 16px',borderRadius:12,border:'1px solid',cursor:'pointer',
              ...
              boxShadow:tab===id?`0 0 0 1px ${color}33`:'var(--shadow-card)',
