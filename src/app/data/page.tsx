...
const LOAD_WEEKS=[{w:'S7',ctl:72,atl:68},{w:'S8',ctl:76,atl:82},{w:'S9',ctl:78,atl:88},{w:'S10',ctl:80,atl:75},{w:'S11',ctl:82,atl:90},{w:'S12',ctl:84,atl:91}]
...
function StatBox({label,value,color}:{label:string;value:string|number;color?:string}){
  return(<div style={{background:'var(--bg-card2)',border:'1px solid var(--border)',borderRadius:12,padding:'11px 13px'}}><p style={{fontSize:10,fontWeight:600,textTransform:'uppercase' as const,letterSpacing:'0.07em',color:'var(--text-dim)',margin:'0 0 4px'}}>{label}</p><p style={{fontFamily:'Syne,sans-serif',fontSize:20,fontWeight:700,color:color||'var(--text)',margin:0}}>{value}</p></div>)
}

function PeriodSel({value,onChange}:{value:Period;onChange:(p:Period)=>void}){
  return(<div style={{display:'flex',gap:3,flexWrap:'wrap' as const}}>{(['1S','6S','1M','3M','6M','1A'] as Period[]).map(id=>(<button key={id} onClick={()=>onChange(id)} style={{padding:'3px 9px',borderRadius:6,border:'1px solid',fontSize:10,cursor:'pointer',borderColor:value===id?'#00c8e0':'var(--border)',background:value===id?'rgba(0,200,224,0.10)':'var(--bg-card2)',color:value===id?'#00c8e0':'var(--text-dim)',fontWeight:value===id?600:400}}>{id}</button>))}</div>)
}
...
function LineChart({data,color='#00c8e0',height=60}:{data:number[];color?:string;height?:number}){
  ...
  return(<svg viewBox={`0 0 ${w} ${height}`} style={{width:'100%',height,overflow:'visible'}}><defs><linearGradient id={`lg${color.replace('#','')}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={color} stopOpacity="0.3"/><stop offset="100%" stopColor={color} stopOpacity="0.02"/></linearGradient></defs><polygon points={`0,${height} ${pts} ${w},${height}`} fill={`url(#lg${color.replace('#','')})`}/><polyline points={pts} fill="none" stroke={color} strokeWidth="1.8" strokeLinejoin="round" strokeLinecap="round"/></svg>)
}
...
function TrainingTab(){
  ...
  const sports=[{k:'run',l:'Running',c:'#22c55e',v:d.sports.run},{k:'bike',l:'Cyclisme',c:'#3b82f6',v:d.sports.bike},{k:'swim',l:'Natation',c:'#38bdf8',v:d.sports.swim},{k:'hyrox',l:'Hyrox',c:'#ef4444',v:d.sports.hyrox},{k:'gym',l:'Musculation',c:'#f97316',v:d.sports.gym}].filter(s=>s.v>0)
  const tLabels=['Récup','EF','SL1','Sweet Spot','SL2','VMA','Mixte']
  const tColors=['#9ca3af','#22c55e','#38bdf8','#eab308','#f97316','#ef4444','#a78bfa']
  const maxLoad=Math.max(...LOAD_WEEKS.map(w=>Math.max(w.ctl,w.atl)))
  const card={background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:16,padding:20,boxShadow:'var(--shadow-card)'}
  return(
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      <div style={card}>
        <h2 style={{fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:700,margin:'0 0 12px'}}>Charge — CTL / ATL / TSB</h2>
        ...
        <div style={{display:'flex',gap:12,marginTop:8}}>
          {[{c:'#00c8e0',l:'CTL'},{c:'#ff5f5f',l:'ATL'},{c:'#5b6fff',l:'TSB'}].map(x=>(<span key={x.l} style={{display:'inline-flex',alignItems:'center',gap:4,fontSize:10,color:x.c}}><span style={{width:8,height:8,borderRadius:2,background:x.c,display:'inline-block'}}/>{x.l}</span>))}
        </div>
      </div>
      <div style={card}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap' as const,gap:8}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:16,flexWrap:'wrap' as const}}>
          <Donut slices={sports.map(s=>({pct:s.v/sportTotal*100,color:s.c}))} size={84} stroke={10}/>
          <div style={{flex:1,display:'flex',flexDirection:'column',gap:7}}>
            {sports.map(s=>{const pct=Math.round(s.v/sportTotal*100);return(<div key={s.k} style={{display:'flex',alignItems:'center',gap:8}}><span style={{width:8,height:8,borderRadius:2,background:s.c,flexShrink:0,display:'inline-block'}}/><span style={{flex:1,fontSize:12,color:'var(--text-mid)'}}>{s.l}</span><div style={{flex:2,height:5,borderRadius:999,overflow:'hidden',background:'var(--border)'}}><div style={{height:'100%',width:`${pct}%`,background:s.c,opacity:0.8,borderRadius:999}}/></div><span style={{fontSize:11,fontFamily:'DM Mono,monospace',color:s.c,width:30,textAlign:'right' as const}}>{pct}%</span></div>)})}
          </div>
        </div>
      </div>
      <div style={card}>
        ...
        <div style={{display:'flex',gap:6,marginBottom:12}}>
          {(['run','bike'] as SportZone[]).map(s=>(<button key={s} onClick={()=>setZoneSport(s)} style={{padding:'5px 12px',borderRadius:8,border:'1px solid',cursor:'pointer',borderColor:zoneSport===s?(s==='run'?'#22c55e':'#3b82f6'):'var(--border)',background:zoneSport===s?(s==='run'?'rgba(34,197,94,0.10)':'rgba(59,130,246,0.10)'):'var(--bg-card2)',color:zoneSport===s?(s==='run'?'#22c55e':'#3b82f6'):'var(--text-dim)',fontSize:12,fontWeight:zoneSport===s?600:400}}>{s==='run'?'🏃 Running':'🚴 Cyclisme'}</button>))}
        </div>
        <div style={{display:'flex',gap:3,height:12,borderRadius:6,overflow:'hidden',marginBottom:8}}>{d.zones[zoneSport].map((pct,i)=><div key={i} style={{flex:pct,background:ZONE_COLORS[i],opacity:0.85}}/>)}</div>
        <div style={{display:'flex',gap:10,flexWrap:'wrap' as const}}>{d.zones[zoneSport].map((pct,i)=>(<div key={i} style={{display:'flex',alignItems:'center',gap:3}}><span style={{width:7,height:7,borderRadius:2,background:ZONE_COLORS[i],display:'inline-block'}}/><span style={{fontSize:10,color:ZONE_COLORS[i],fontFamily:'DM Mono,monospace',fontWeight:600}}>{ZONE_LABELS[i]} {pct}%</span></div>))}</div>
      </div>
      <div style={card}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap' as const,gap:8}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:16,flexWrap:'wrap' as const}}>
          <Donut slices={d.types.map((pct,i)=>({pct,color:tColors[i]}))} size={80} stroke={10}/>
          <div style={{flex:1,display:'grid',gridTemplateColumns:'1fr 1fr',gap:'5px 12px'}}>
            {tLabels.map((t,i)=>(<div key={t} style={{display:'flex',alignItems:'center',gap:6}}><span style={{width:7,height:7,borderRadius:2,background:tColors[i],display:'inline-block',flexShrink:0}}/><span style={{fontSize:11,color:'var(--text-mid)',flex:1}}>{t}</span><span style={{fontSize:11,fontFamily:'DM Mono,monospace',color:tColors[i],fontWeight:600}}>{d.types[i]}%</span></div>))}
          </div>
        </div>
      </div>
    </div>
...
function RecoveryTab(){
  const [hrvP,setHrvP]=useState<'7j'|'30j'>('7j')
  const hrvData=hrvP==='7j'?HRV_DATA.slice(-7):HRV_DATA.slice(-30)
  ...
  const r=75, rc=r>=70?'#00c8e0':r>=50?'#ffb340':'#ef4444'
  const rl=r>=70?'Bonne forme':r>=50?'Fatigue modérée':'Fatigue élevée'
  const card={background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:16,padding:20,boxShadow:'var(--shadow-card)'}
  return(
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      <div style={{...card,border:`1px solid ${rc}33`}}>
        <div style={{display:'flex',alignItems:'center',gap:16,flexWrap:'wrap' as const}}>
          <Donut slices={[{pct:r,color:rc}]} size={80} stroke={9} center={<div style={{textAlign:'center' as const}}><span style={{fontFamily:'Syne,sans-serif',fontSize:20,fontWeight:700,color:rc}}>{r}</span><span style={{fontSize:8,color:'var(--text-dim)',display:'block'}}>/100</span></div>}/>
          <div style={{flex:1}}>
      ...
      <div style={card}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:10,flexWrap:'wrap' as const,gap:8}}>
          <div><h2 style={{fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:700,margin:0}}>HRV (VFC)</h2><p style={{fontSize:11,color:'var(--text-dim)',margin:'2px 0 0'}}>Moy. <span style={{color:'#00c8e0',fontWeight:600,fontFamily:'DM Mono,monospace'}}>{avgHrv}ms</span> · <span style={{color:'#22c55e'}}>↑ Positive</span></p></div>
          <div style={{display:'flex',gap:4}}>{(['7j','30j'] as const).map(p=>(<button key={p} onClick={()=>setHrvP(p)} style={{padding:'4px 10px',borderRadius:7,border:'1px solid',fontSize:11,cursor:'pointer',borderColor:hrvP===p?'#00c8e0':'var(--border)',background:hrvP===p?'rgba(0,200,224,0.10)':'var(--bg-card2)',color:hrvP===p?'#00c8e0':'var(--text-dim)'}}>{p}</button>))}</div>
        </div>
        <LineChart data={hrvData} color="#00c8e0" height={56}/>
        <div style={{display:'flex',justifyContent:'space-between',marginTop:4,fontSize:10,color:'var(--text-dim)'}}><span>-{hrvData.length}j</span><span>Aujourd'hui</span></div>
      </div>
      <div style={card}>
        ...
        <div style={{display:'flex',alignItems:'flex-end',gap:3,height:52}}>{SLEEP_DATA.slice(-14).map((v,i)=>{const c=v>=7.5?'#22c55e':v>=6.5?'#ffb340':'#ef4444';return<div key={i} style={{flex:1,height:`${(v/9)*52}px`,minHeight:2,borderRadius:'3px 3px 0 0',background:`linear-gradient(180deg,${c}bb,${c}44)`}}/>})}</div>
        <div style={{display:'flex',justifyContent:'space-between',marginTop:4,fontSize:10,color:'var(--text-dim)'}}><span>-14j</span><span>Aujourd'hui</span></div>
      </div>
      <div style={card}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginTop:4,fontSize:10,color:'var(--text-dim)'}}><span>-30j</span><span>Aujourd'hui</span></div>
      </div>
    </div>
  )
}
...
export default function DataPage(){
  ...
  const TABS:[MainTab,string,string,string][]=[['training','Training','#3b82f6','rgba(59,130,246,0.10)'],['recovery','Recovery','#22c55e','rgba(34,197,94,0.10)'],['nutrition','Nutrition','#ffb340','rgba(255,179,64,0.10)']]
  return(
    <div style={{padding:'24px 28px',maxWidth:'100%'}}>
      <div style={{marginBottom:20}}><h1 style={{fontFamily:'Syne,sans-serif',fontSize:26,fontWeight:700,letterSpacing:'-0.03em',margin:0}}>Données & Analyse</h1><p style={{fontSize:12.5,color:'var(--text-dim)',margin:'5px 0 0'}}>Training · Recovery · Nutrition</p></div>
      <div style={{display:'flex',gap:8,marginBottom:20,flexWrap:'wrap' as const}}>
        {TABS.map(([id,label,color,bg])=>(<button key={id} onClick={()=>setTab(id)} style={{flex:1,minWidth:100,padding:'11px 16px',borderRadius:12,border:'1px solid',cursor:'pointer',borderColor:tab===id?color:'var(--border)',background:tab===id?bg:'var(--bg-card)',color:tab===id?color:'var(--text-mid)',fontFamily:'Syne,sans-serif',fontSize:13,fontWeight:tab===id?700:400,boxShadow:tab===id?`0 0 0 1px ${color}33`:'var(--shadow-card)',transition:'all 0.15s'}}>{label}</button>))}
      </div>
