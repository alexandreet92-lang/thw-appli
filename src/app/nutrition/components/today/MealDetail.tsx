...
export function MealDetail({ foods, grouped, onTapFood, onDeleteFood }: {
  ...
  const groups = grouped
    ? COURSE_ORDER
        .map(c => ({ label: c.label, items: indexed.filter(({ f }) => (f.course ?? 'plat') === c.key) }))
          ...
          {groups
            ? groups.map(g => (
                <div key={g.label} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <span style={{ fontFamily: FB, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--text-dim)', marginTop: 2 }}>{g.label}</span>
