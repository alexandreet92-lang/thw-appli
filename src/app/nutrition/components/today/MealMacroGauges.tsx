...
const ROWS = [
  { key: 'prot', label: 'Protéines', color: 'var(--macro-prot)', k: 4 },
  { key: 'gluc', label: 'Glucides',  color: 'var(--macro-gluc)', k: 4 },
  { key: 'lip',  label: 'Lipides',   color: 'var(--macro-lip)',  k: 9 },
] as const
        ...
        return (
          <div key={r.key} style={{ width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 'var(--space-2)', marginBottom: 'var(--space-1)' }}>
              <span style={{ fontFamily: FB, fontSize: 12, color: 'var(--text-mid)', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.label}</span>
