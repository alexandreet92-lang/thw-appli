      ...
      <div onClick={e => e.stopPropagation()} className={closing ? 'sheet-close' : 'sheet-open'}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--border)', gap: 8 }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 17, fontWeight: 600, color: 'var(--text)', margin: 0 }}>
            {t('performance.linkActivityTitle', { segment: SEG_LABEL[segment] })}
          </h2>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 20px 24px' }}>
          {acts === null ? (
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--text-dim)' }}>{t('performance.loading')}</p>
          ) : filtered.length === 0 ? (
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--text-dim)' }}>{t('performance.noActivityFound', { segment: SEG_LABEL[segment].toLowerCase() })}</p>
          ) : filtered.map(a => (
            <button key={a.id} onClick={() => { onLink(a); close() }}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                <span style={{ fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.title ?? t('performance.untitled')}</span>
