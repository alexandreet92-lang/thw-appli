    ...
    <div onClick={close} className="rec-drawer" style={{ position: 'fixed', inset: 0, zIndex: 3000, background: SCRIM, display: 'flex', alignItems: 'flex-end' }}>
      <div onClick={e => e.stopPropagation()} className={closing ? 'sheet-close' : 'sheet-open'} style={{ width: '100%', maxHeight: 'calc(100dvh - 72px)', background: 'var(--bg-card)', borderRadius: '20px 20px 0 0', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden', willChange: 'transform' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px 100px' }}>
          <TriSegment title={t('performance.sportSwimming')} distLabel={fmtSwim} dot={SWIM_DOT} segment="swim" time={swim} setTime={setSwim} auto={swimAuto} onLink={a => link('swim', a)} chips={chips.swim} linked={!!chips.swim} />
          <TriSegment title={t('performance.t1Transition')} time={t1} setTime={setT1} timeLabel={t('performance.durationMs')} />
          <TriSegment title={t('performance.sportBike')} distLabel={fmtBike} dot={BIKE_DOT} segment="bike" time={bikeTime} setTime={setBikeTime} timeLabel={t('performance.timeHms')} auto={bikeAuto} onLink={a => link('bike', a)} chips={chips.bike} linked={!!chips.bike}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div><p style={fieldLbl}>{t('performance.avgWatts')}</p>{subInp(bikeWatts, setBikeWatts, t('performance.egWatts'), 'number')}{wkg && <span className="tnum" style={{ display: 'block', marginTop: 5, fontFamily: 'var(--font-body)', fontSize: 11, color: 'var(--text-mid)' }}>→ {wkg}</span>}</div>
              <div><p style={fieldLbl}>{t('performance.npWatts')}</p>{subInp(bikeNP, setBikeNP, t('performance.egNp'), 'number')}</div>
            </div>
            <p style={fieldLbl}>{t('performance.avgHr')}</p>{subInp(bikeHr, setBikeHr, t('performance.egHrBike'), 'number')}
          </TriSegment>
          <TriSegment title={t('performance.t2Transition')} time={t2} setTime={setT2} timeLabel={t('performance.durationMs')} />
          <TriSegment title="Running" distLabel={fmtRun} dot={RUN_DOT} segment="run" time={run} setTime={setRun} auto={runAuto} onLink={a => link('run', a)} chips={chips.run} linked={!!chips.run}>
            <p style={fieldLbl}>{t('performance.avgHr')}</p>{subInp(runHr, setRunHr, t('performance.egHrRun'), 'number')}
          </TriSegment>
          ...
          <div style={{ background: 'var(--bg-card2)', borderRadius: 14, padding: '14px 16px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              {autoTotalStr && autoTotalStr !== displayTotal && (
                <button onClick={() => setDraft(autoTotalStr)} style={{ padding: 0, border: 'none', background: 'transparent', color: 'var(--primary)', fontFamily: 'var(--font-body)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>{t('performance.recalculate', { value: autoTotalStr })}</button>
              )}
            </div>
          </div>
        </div>
