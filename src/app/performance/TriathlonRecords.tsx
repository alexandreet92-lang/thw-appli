...
export function TriathlonRecords({ getBest, getPrev, profile, onEdit }: {
  ...
  const sportBars = [
    { label: 'Nat.', color: SWIM, sec: segSecs[0].sec },
    { label: 'Vélo', color: BIKE, sec: segSecs[2].sec },
    { label: 'Course', color: RUN, sec: segSecs[4].sec },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {/* Décomposition */}
      <div style={card}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 600, color: 'var(--text)', margin: '0 0 16px' }}>Décomposition — {selDef.label}</h2>
        {total === 0 ? (
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--text-dim)', margin: 0 }}>Aucun record sur cette distance. « Modifier » pour saisir une course.</p>
        ) : (
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 18, height: 156 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 40, height: 120, borderRadius: 'var(--r-sm)', overflow: 'hidden', display: 'flex', flexDirection: 'column-reverse', background: 'var(--bg-card2)' }}>
                {segSecs.map(s => (
                  <div key={s.label} title={`${s.label} ${hmsFull(s.sec)}`} style={{ width: '100%', height: mounted ? `${pct(s.sec)}%` : '0%', background: s.color, opacity: s.color === TRANS ? 0.35 : 0.6, transition: 'height 0.9s cubic-bezier(0.32,0.72,0,1)' }} />
                ))}
              </div>
              <span style={{ fontFamily: 'var(--font-body)', fontSize: 10, color: 'var(--text-dim)' }}>Total</span>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 12px', marginTop: 14 }}>
          {[{ l: 'Natation', c: SWIM }, { l: 'Vélo', c: BIKE }, { l: 'Course', c: RUN }, { l: 'Transitions', c: TRANS }].map(x => (
      ...
      <div style={card}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 600, color: 'var(--text)', margin: '0 0 12px' }}>Records par distance</h2>
        {DISTS.map(d => {
          const b = getBest(d.id), prev = getPrev(d.id)
          const splits = SEGS.map(s => ({ label: s.label, v: (b?.[s.key] as string) ?? null })).filter(s => s.v)
          return (
            <div key={d.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px 8px', borderRadius: 'var(--r-sm)', marginBottom: 4, background: d.id === sel ? 'var(--bg-card2)' : 'transparent' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {prev && <span className="tnum" style={{ fontFamily: 'var(--font-body)', fontSize: 10, color: 'var(--text-dim)' }}>Préc. : {prev.performance}{prev.achieved_at ? ` (${prev.achieved_at.slice(0, 4)})` : ''}</span>}
              </div>
              <button onClick={() => onEdit(d.id)} style={{ flexShrink: 0, padding: 0, border: 'none', background: 'transparent', color: 'var(--primary)', fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>Modifier</button>
            </div>
          )
        })}
      </div>
