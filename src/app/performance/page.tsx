...
function ProgressionTab() {
  ...
  const items = [
    { label:'FTP velo',       v24:`${PROG.ftp[2]}W`,     v23:`${PROG.ftp[1]}W`,     v22:`${PROG.ftp[0]}W`,     gain:pctChange(PROG.ftp[1],PROG.ftp[2]),                                              color:'#00c8e0', good:true  },
    { label:'5km course',     v24:PROG.run5k[2],          v23:PROG.run5k[1],          v22:PROG.run5k[0],          gain:pctChange(parseSec(PROG.run5k[1]),parseSec(PROG.run5k[2])),                     color:'#22c55e', good:false },
    { label:'100m natation',  v24:PROG.swim100[2],        v23:PROG.swim100[1],        v22:PROG.swim100[0],        gain:pctChange(parseSec(PROG.swim100[1]),parseSec(PROG.swim100[2])),                  color:'#38bdf8', good:false },
    { label:'2000m aviron',   v24:PROG.row2k[2],          v23:PROG.row2k[1],          v22:PROG.row2k[0],          gain:pctChange(parseSec(PROG.row2k[1]),parseSec(PROG.row2k[2])),                     color:'#14b8a6', good:false },
  ]
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      <Card>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {items.map(item => {
            const isGood = item.good ? item.gain.startsWith('+') : item.gain.startsWith('-')
      ...
      <Card>
        ...
        <div style={{ marginTop:12, padding:'10px 14px', borderRadius:10, background:'rgba(255,179,64,0.08)', border:'1px solid rgba(255,179,64,0.20)' }}>
          ...
          <p style={{ fontSize:11, color:'var(--text-mid)', margin:0 }}>
            Plateau en musculation detecte. Essaie d'ajouter 2.5kg par semaine sur tes mouvements principaux.
          </p>
        </div>
      </Card>
    </div>
