cat > /mnt/user-data/outputs/performance-page.tsx << 'ENDOFFILE'
'use client'
...
function parsePaceSec(pace:string):number { const p=pace.split(':'); return parseInt(p[0])*60+(parseInt(p[1])||0) }
function secToPace(sec:number):string { return `${Math.floor(sec/60)}:${String(sec%60).padStart(2,'0')}` }
...
const GYM_MOVES=[
  ...
  {name:'Dév. militaire',recs:[{l:'Max charge',v:80}]},
  {name:'Pompes',     recs:[{l:'Max reps',v:65}]},
]

// ── UI atoms ──────────────────────────────────────
function Card({children,style}:{children:React.ReactNode;style?:React.CSSProperties}) {
  return <div style={{background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:16,padding:20,boxShadow:'var(--shadow-card)',...style}}>{children}</div>
}
      ...
      {zones.map((z,i)=>(
        <div key={z.z} style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{height:5,borderRadius:999,background:`${Z_COLORS[i]}22`}}><div style={{height:'100%',width:`${20+i*16}%`,background:Z_COLORS[i],opacity:0.7,borderRadius:999}}/></div>
          </div>
        </div>
      ))}
    </div>
...
function NInput({label,value,onChange,unit,step}:{label:string;value:number;onChange:(v:number)=>void;unit?:string;step?:number}) {
  return (<div><p style={{fontSize:10,fontWeight:600,textTransform:'uppercase' as const,letterSpacing:'0.06em',color:'var(--text-dim)',marginBottom:4}}>{label}{unit&&<span style={{fontWeight:400,marginLeft:3,textTransform:'none' as const}}>({unit})</span>}</p><input type="number" value={value} step={step||1} onChange={e=>onChange(parseFloat(e.target.value)||0)} style={{width:'100%',padding:'7px 10px',borderRadius:8,border:'1px solid var(--border)',background:'var(--input-bg)',color:'var(--text)',fontFamily:'DM Mono,monospace',fontSize:12,outline:'none'}}/></div>)
}
function TInput({label,value,onChange,placeholder}:{label:string;value:string;onChange:(v:string)=>void;placeholder?:string}) {
  return (<div><p style={{fontSize:10,fontWeight:600,textTransform:'uppercase' as const,letterSpacing:'0.06em',color:'var(--text-dim)',marginBottom:4}}>{label}</p><input type="text" value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} style={{width:'100%',padding:'7px 10px',borderRadius:8,border:'1px solid var(--border)',background:'var(--input-bg)',color:'var(--text)',fontFamily:'DM Mono,monospace',fontSize:12,outline:'none'}}/></div>)
}

// ── Power curve ───────────────────────────────────
function PowerCurve({d24,d23}:{d24:number[];d23:number[]}) {
  const maxW=Math.max(...d24,...d23)*1.05; const W=400,H=110
  function pts(arr:number[]) { return arr.map((v,i)=>`${(i/(arr.length-1))*W},${H-((v/maxW)*H)}`).join(' ') }
  const p24=pts(d24),p23=pts(d23)
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%',height:H+4,overflow:'visible'}}>
      <defs>
        <linearGradient id="g24" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#00c8e0" stopOpacity="0.25"/><stop offset="100%" stopColor="#00c8e0" stopOpacity="0.02"/></linearGradient>
        <linearGradient id="g23" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#5b6fff" stopOpacity="0.15"/><stop offset="100%" stopColor="#5b6fff" stopOpacity="0.02"/></linearGradient>
      </defs>
...
function LineChart({series,labels}:{series:{label:string;color:string;data:number[]}[];labels:string[]}) {
  const all=series.flatMap(s=>s.data); const min=Math.min(...all)*0.95,max=Math.max(...all)*1.05
  const W=300,H=80
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%',height:H,overflow:'visible'}}>
        {series.map(s=>{const pts=s.data.map((v,i)=>`${(i/(s.data.length-1))*W},${H-((v-min)/(max-min))*H}`).join(' ');return <polyline key={s.label} points={pts} fill="none" stroke={s.color} strokeWidth="2" strokeLinejoin="round" strokeLinecap="round"/>})}
        {series.map(s=>s.data.map((v,i)=><circle key={`${s.label}${i}`} cx={(i/(s.data.length-1))*W} cy={H-((v-min)/(max-min))*H} r="3" fill={s.color}/>))}
      </svg>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      <Card>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap' as const,gap:8}}>
          <div><h2 style={{fontFamily:'Syne,sans-serif',fontSize:15,fontWeight:700,margin:0}}>Profil athlète</h2><p style={{fontSize:11,color:'var(--text-dim)',margin:'2px 0 0'}}>Paramètres physiologiques de base</p></div>
          <button onClick={()=>setEditing(!editing)} style={{padding:'6px 14px',borderRadius:9,background:editing?'linear-gradient(135deg,#00c8e0,#5b6fff)':'var(--bg-card2)',border:`1px solid ${editing?'transparent':'var(--border)'}`,color:editing?'#fff':'var(--text-mid)',fontSize:12,cursor:'pointer',fontWeight:600}}>{editing?'✓ Sauvegarder':'✏️ Modifier'}</button>
        </div>
        {editing?(
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10}} className="md:grid-cols-3">
            ...
            <NInput label="Âge" value={profile.age} onChange={v=>setProfile({...profile,age:v})}/>
            ...
            <NInput label="VO2max estimé" value={profile.vo2max} onChange={v=>setProfile({...profile,vo2max:v})}/>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10}} className="md:grid-cols-4">
            ...
            <StatBox label="VO2max estimé" value={profile.vo2max} unit="ml/kg/min" color="#a855f7"/>
          </div>
        )}
      </Card>
      <Card>
        ...
        {[
          {label:'Puissance relative (W/kg)',val:parseFloat(wkg),max:6,unit:'W/kg',color:'#00c8e0',desc:parseFloat(wkg)>=4.5?'Expert':parseFloat(wkg)>=3.5?'Avancé':parseFloat(wkg)>=2.5?'Intermédiaire':'Débutant'},
          {label:'VO2max estimé',val:profile.vo2max,max:80,unit:'ml/kg/min',color:'#a855f7',desc:profile.vo2max>=65?'Elite':profile.vo2max>=55?'Élevé':profile.vo2max>=40?'Moyen':'Faible'},
          {label:'FC repos',val:100-profile.hrRest,max:70,unit:'',color:'#22c55e',desc:profile.hrRest<=40?'Elite':profile.hrRest<=50?'Élevé':profile.hrRest<=60?'Moyen':'Faible'},
        ].map(item=>(
          <div key={item.label} style={{marginBottom:12}}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
              ...
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                ...
                <span style={{fontFamily:'DM Mono,monospace',fontSize:12,fontWeight:700,color:item.color}}>{item.label.includes('FC')?profile.hrRest+'bpm':item.val} {item.unit}</span>
              </div>
            </div>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      <Card>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14,flexWrap:'wrap' as const,gap:8}}>
          <h2 style={{fontFamily:'Syne,sans-serif',fontSize:15,fontWeight:700,margin:0}}>Zones d'entraînement</h2>
          <button onClick={()=>setEditing(!editing)} style={{padding:'5px 12px',borderRadius:8,background:editing?'linear-gradient(135deg,#00c8e0,#5b6fff)':'var(--bg-card2)',border:`1px solid ${editing?'transparent':'var(--border)'}`,color:editing?'#fff':'var(--text-mid)',fontSize:11,cursor:'pointer',fontWeight:600}}>{editing?'✓ Appliquer':'✏️ Modifier seuils'}</button>
        </div>
        {editing&&(
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap' as const}}>
          {([['power','⚡ Puissance'],['pace','🏃 Allures'],['hr','❤️ Fréquence cardiaque']] as [ZoneTab,string][]).map(([t,l])=>(
            <button key={t} onClick={()=>setZoneTab(t)} style={{padding:'6px 13px',borderRadius:9,border:'1px solid',cursor:'pointer',borderColor:zoneTab===t?'#00c8e0':'var(--border)',background:zoneTab===t?'rgba(0,200,224,0.10)':'var(--bg-card2)',color:zoneTab===t?'#00c8e0':'var(--text-mid)',fontSize:12,fontWeight:zoneTab===t?600:400}}>{l}</button>
          ))}
        </div>
      </Card>
      {zoneTab==='power'&&(
        <div style={{display:'grid',gridTemplateColumns:'1fr',gap:12}} className="md:grid-cols-2">
          ...
          <Card>
            ...
            <ZBars zones={[
              {z:'Z1',label:'Récup',  range:'> 2:15/500m'},
              {z:'Z2',label:'Aérobie',range:'2:00 – 2:14/500m'},
        ...
        <Card>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14,flexWrap:'wrap' as const,gap:8}}>
            ...
            <div style={{display:'flex',gap:12,fontSize:11}}>
              <span>Repos : <strong style={{color:'#22c55e',fontFamily:'DM Mono,monospace'}}>{profile.hrRest}bpm</strong></span>
              <span>LTHR : <strong style={{color:'#f97316',fontFamily:'DM Mono,monospace'}}>{profile.lthr}bpm</strong></span>
              <span>Max : <strong style={{color:'#ef4444',fontFamily:'DM Mono,monospace'}}>{profile.hrMax}bpm</strong></span>
            </div>
          </div>
          ...
          <div style={{marginTop:14}}>
            ...
            <div style={{display:'flex',marginTop:3}}>
              {hrZones.map((z,i)=><div key={z.z} style={{flex:z.max-z.min,textAlign:'center' as const}}><span style={{fontSize:9,fontFamily:'DM Mono,monospace',color:Z_COLORS[i]}}>{z.min}</span></div>)}
...
function RecordsTab() {
  ...
  const [bikeRec,setBikeRec]=useState(BIKE_REC)
  const [runRec,setRunRec]=useState(RUN_REC)
  const [swimRec,setSwimRec]=useState(SWIM_REC)
  const [rowRec,setRowRec]=useState(ROW_REC)
  const [gymRec,setGymRec]=useState(GYM_MOVES)
  const [editVal,setEditVal]=useState<Record<string,string>>({})

  const curve24=BIKE_DURS.map(d=>bikeRec[d]?.[0]?.w||0)
  const curve23=BIKE_DURS.map(d=>bikeRec[d]?.[1]?.w||0)
  ...
  const SPORT_TABS:[RecordSport,string,string][]=[
    ['bike','🚴 Vélo','#3b82f6'],['run','🏃 Course','#22c55e'],['swim','🏊 Natation','#38bdf8'],
    ['rowing','🚣 Aviron','#14b8a6'],['hyrox','🏋️ Hyrox','#ef4444'],['gym','💪 Muscu','#f97316'],
  ]
  ...
  function RecordRow({label,rec24,rec23,sub24,pr}:{label:string;rec24:string;rec23:string;sub24?:string;pr?:boolean}) {
    return (
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',borderRadius:9,background:'var(--bg-card2)',border:'1px solid var(--border)',marginBottom:5}}>
        <span style={{fontSize:11,fontWeight:500,color:'var(--text-mid)',minWidth:70,flexShrink:0}}>{label}</span>
        <div style={{flex:1}}>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      <div style={{display:'flex',gap:5,flexWrap:'wrap' as const}}>
        {SPORT_TABS.map(([s,l,c])=>(
          <button key={s} onClick={()=>setSport(s)} style={{padding:'7px 12px',borderRadius:9,border:'1px solid',cursor:'pointer',borderColor:sport===s?c:'var(--border)',background:sport===s?`${c}22`:'var(--bg-card)',color:sport===s?c:'var(--text-mid)',fontSize:12,fontWeight:sport===s?600:400}}>{l}</button>
        ))}
      </div>
      ...
      {sport==='bike'&&(
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          ...
          <Card>
            ...
            {BIKE_DURS.map(d=>{
              const r24=bikeRec[d]?.[0],r23=bikeRec[d]?.[1]
              const pr=r24&&r23&&r24.w>r23.w
              return r24&&<RecordRow key={d} label={d} rec24={`${r24.w}W`} rec23={r23?`${r23.w}W`:'—'} sub24={`${(r24.w/INIT_PROFILE.weight).toFixed(2)} W/kg`} pr={pr}/>
            })}
          </Card>
        </div>
      )}
      ...
      {sport==='run'&&(
        <Card>
          ...
          {RUN_DISTS.map(d=>{
            const r24=runRec[d]?.[0],r23=runRec[d]?.[1]
      ...
      {sport==='swim'&&(
        <Card>
          ...
          {SWIM_DISTS.map(d=>{
            const r24=swimRec[d]?.[0],r23=swimRec[d]?.[1]
            const pace24=r24?.time&&r24.time!=='—'?`${calcPaceStr(SWIM_M[d]/1000,r24.time).replace('/km','')}/100m`:'—'
            return <RecordRow key={d} label={d} rec24={r24?.time||'—'} rec23={r23?.time||'—'} sub24={pace24!=='—'?pace24:undefined}/>
          })}
        </Card>
      )}
      ...
      {sport==='rowing'&&(
        <Card>
          ...
          {ROW_DISTS.map(d=>{
            const r24=rowRec[d]?.[0],r23=rowRec[d]?.[1]
            const split=calcSplit500(ROW_M[d],r24?.time||'')
            const watts=split!=='—'?(() => { const p=split.split('/')[0].split(':').map(Number); const s=p[0]*60+(p[1]||0); return `~${Math.round(2.80/(s/500)**3)}W` })():'—'
            return <RecordRow key={d} label={d==='Semi'?'Semi (21km)':d==='Marathon'?'Marathon (42km)':d} rec24={r24?.time||'—'} rec23={r23?.time||'—'} sub24={split!=='—'?`${split} · ${watts}`:undefined}/>
          })}
          <p style={{fontSize:10,color:'var(--text-dim)',margin:'10px 0 0'}}>Puissance estimée basée sur formule Concept2 : P = 2.80 / (split/500)³</p>
        </Card>
      )}
          ...
          <Card>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
              ...
              <div style={{padding:'8px 12px',borderRadius:9,background:'var(--bg-card2)',border:'1px solid var(--border)'}}>
                <p style={{fontSize:10,color:'var(--text-dim)',margin:'0 0 3px'}}>Temps de run total</p>
            ...
            <h3 style={{fontSize:12,fontWeight:600,color:'var(--text-dim)',textTransform:'uppercase' as const,letterSpacing:'0.07em',margin:'0 0 8px'}}>Stations</h3>
            <div style={{display:'flex',flexDirection:'column',gap:5,marginBottom:12}}>
              {HYROX_STATIONS.map((s,i)=>(
                <div key={s} style={{display:'flex',alignItems:'center',gap:10,padding:'6px 10px',borderRadius:8,background:'rgba(239,68,68,0.05)',border:'1px solid rgba(239,68,68,0.12)'}}>
                  <span style={{fontSize:10,fontWeight:700,color:'#ef4444',width:18,flexShrink:0}}>{i+1}</span>
            ...
            <h3 style={{fontSize:12,fontWeight:600,color:'var(--text-dim)',textTransform:'uppercase' as const,letterSpacing:'0.07em',margin:'0 0 8px'}}>Runs (8×1km)</h3>
          ...
          <Card>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                {simMode&&<div style={{padding:'8px 14px',borderRadius:9,background:'rgba(239,68,68,0.10)',border:'1px solid rgba(239,68,68,0.25)'}}>
                ...
                <button onClick={()=>setSimMode(!simMode)} style={{padding:'6px 12px',borderRadius:9,background:simMode?'linear-gradient(135deg,#ef4444,#f97316)':'var(--bg-card2)',border:`1px solid ${simMode?'transparent':'var(--border)'}`,color:simMode?'#fff':'var(--text-mid)',fontSize:11,cursor:'pointer',fontWeight:600}}>{simMode?'✓ Simulation active':'▷ Simuler'}</button>
              </div>
            </div>
            {simMode&&(
                ...
                {HYROX_STATIONS.map(s=>(
                  <div key={s} style={{display:'flex',alignItems:'center',gap:10}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:4}}>
                      <button onClick={()=>setSimDeltas(p=>({...p,[s]:(p[s]||0)+5}))} style={{width:24,height:24,borderRadius:5,border:'1px solid var(--border)',background:'var(--bg-card2)',color:'#22c55e',cursor:'pointer',fontSize:14,display:'flex',alignItems:'center',justifyContent:'center'}}>−</button>
                      ...
                      <button onClick={()=>setSimDeltas(p=>({...p,[s]:Math.max((p[s]||0)-5,0)}))} style={{width:24,height:24,borderRadius:5,border:'1px solid var(--border)',background:'var(--bg-card2)',color:'#ef4444',cursor:'pointer',fontSize:14,display:'flex',alignItems:'center',justifyContent:'center'}}>+</button>
                    </div>
                  </div>
                ))}
                <button onClick={()=>setSimDeltas({})} style={{marginTop:8,padding:'6px',borderRadius:8,background:'var(--bg-card2)',border:'1px solid var(--border)',color:'var(--text-dim)',fontSize:11,cursor:'pointer'}}>Réinitialiser</button>
              </div>
            )}
            {!simMode&&<p style={{fontSize:12,color:'var(--text-dim)',textAlign:'center' as const,padding:'10px 0'}}>Active la simulation pour identifier tes points faibles et calculer le gain potentiel.</p>}
          </Card>
        </div>
      )}
              ...
              <div style={{display:'flex',flexDirection:'column',gap:5}}>
                {m.recs.map(r=>(
                  <div key={r.l} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'5px 9px',borderRadius:7,background:'rgba(249,115,22,0.07)',border:'1px solid rgba(249,115,22,0.15)'}}>
                    ...
                    <span style={{fontFamily:'DM Mono,monospace',fontSize:13,fontWeight:700,color:r.v?'#f97316':'var(--text-dim)'}}>{r.v?`${r.v}${r.l.includes('reps')?'':r.l.includes('charge')?'kg':r.l==='Max reps PDC'?'':' kg'}`:'—'}</span>
                  </div>
                ))}
              </div>
            </Card>
...
function ProgressionTab() {
  ...
  const ftpSeries=[{label:'FTP',color:'#00c8e0',data:[265,285,301]}]
  const run5kSec=[{label:'5km',color:'#22c55e',data:[1185,1092,1065]}]
  const swim100Sec=[{label:'100m',color:'#38bdf8',data:[82,76,70]}]
  const row2kSec=[{label:'2000m',color:'#14b8a6',data:[455,428,412]}]

  function pct(a:number,b:number):string { return `+${((b-a)/a*100).toFixed(1)}%` }
  function pctInv(a:number,b:number):string { return `${((b-a)/a*100).toFixed(1)}%` }

  const progressItems=[
    {label:'FTP vélo',v24:`${PROG_DATA.ftp[2]}W`,v23:`${PROG_DATA.ftp[1]}W`,v22:`${PROG_DATA.ftp[0]}W`,gain:pct(PROG_DATA.ftp[1],PROG_DATA.ftp[2]),color:'#00c8e0',positive:true},
    {label:'5km course',v24:PROG_DATA.run5k[2],v23:PROG_DATA.run5k[1],v22:PROG_DATA.run5k[0],gain:pctInv(parsePaceSec(PROG_DATA.run5k[1]),parsePaceSec(PROG_DATA.run5k[2])),color:'#22c55e',positive:false},
    {label:'100m natation',v24:PROG_DATA.swim100[2],v23:PROG_DATA.swim100[1],v22:PROG_DATA.swim100[0],gain:pctInv(parsePaceSec(PROG_DATA.swim100[1]),parsePaceSec(PROG_DATA.swim100[2])),color:'#38bdf8',positive:false},
    {label:'2000m aviron',v24:PROG_DATA.row2k[2],v23:PROG_DATA.row2k[1],v22:PROG_DATA.row2k[0],gain:pctInv(parsePaceSec(PROG_DATA.row2k[1]),parsePaceSec(PROG_DATA.row2k[2])),color:'#14b8a6',positive:false},
  ]
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      <Card>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {progressItems.map(item=>{
            const isGood=item.positive?item.gain.startsWith('+'):item.gain.startsWith('-')
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr',gap:12}} className="md:grid-cols-2">
        <Card>
          <h3 style={{fontFamily:'Syne,sans-serif',fontSize:13,fontWeight:700,margin:'0 0 12px'}}>🚴 FTP Vélo — évolution</h3>
          <LineChart series={ftpSeries} labels={['2022','2023','2024']}/>
          <div style={{display:'flex',gap:8,marginTop:6}}>
            {PROG_DATA.ftp.map((v,i)=><span key={i} style={{fontSize:11,fontFamily:'DM Mono,monospace',color:'#00c8e0'}}>{['2022','2023','2024'][i]}: <strong>{v}W</strong></span>)}
          </div>
        </Card>
        <Card>
          <h3 style={{fontFamily:'Syne,sans-serif',fontSize:13,fontWeight:700,margin:'0 0 12px'}}>🏃 5km — évolution</h3>
          <LineChart series={run5kSec.map(s=>({...s,data:s.data.map(v=>1000-v)}))} labels={['2022','2023','2024']}/>
          <div style={{display:'flex',gap:8,marginTop:6}}>
            {PROG_DATA.run5k.map((v,i)=><span key={i} style={{fontSize:11,fontFamily:'DM Mono,monospace',color:'#22c55e'}}>{['2022','2023','2024'][i]}: <strong>{v}</strong></span>)}
          </div>
        </Card>
        <Card>
          <h3 style={{fontFamily:'Syne,sans-serif',fontSize:13,fontWeight:700,margin:'0 0 12px'}}>🏊 100m natation — évolution</h3>
          <LineChart series={swim100Sec.map(s=>({...s,data:s.data.map(v=>200-v)}))} labels={['2022','2023','2024']}/>
          <div style={{display:'flex',gap:8,marginTop:6}}>
            {PROG_DATA.swim100.map((v,i)=><span key={i} style={{fontSize:11,fontFamily:'DM Mono,monospace',color:'#38bdf8'}}>{['2022','2023','2024'][i]}: <strong>{v}</strong></span>)}
          </div>
        </Card>
        <Card>
          <h3 style={{fontFamily:'Syne,sans-serif',fontSize:13,fontWeight:700,margin:'0 0 12px'}}>🚣 2000m aviron — évolution</h3>
          <LineChart series={row2kSec.map(s=>({...s,data:s.data.map(v=>600-v)}))} labels={['2022','2023','2024']}/>
          <div style={{display:'flex',gap:8,marginTop:6}}>
            {PROG_DATA.row2k.map((v,i)=><span key={i} style={{fontSize:11,fontFamily:'DM Mono,monospace',color:'#14b8a6'}}>{['2022','2023','2024'][i]}: <strong>{v}</strong></span>)}
          </div>
        </Card>
      </div>
      ...
      <Card>
        <h2 style={{fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:700,margin:'0 0 12px'}}>🔍 Détection de stagnation</h2>
        <div style={{display:'flex',flexDirection:'column',gap:7}}>
          {[
            {label:'FTP vélo',trend:'↑ Progression constante',color:'#22c55e',status:'En progression'},
            {label:'5km course',trend:'↑ Bonne amélioration',color:'#22c55e',status:'En progression'},
            {label:'Natation',trend:'↑ Progression régulière',color:'#22c55e',status:'En progression'},
            {label:'Aviron',trend:'↑ Bonne dynamique',color:'#22c55e',status:'En progression'},
            {label:'Musculation',trend:'→ Stable depuis 6 mois',color:'#ffb340',status:'Plateau détecté'},
          ].map(x=>(
        ...
        <div style={{marginTop:12,padding:'10px 14px',borderRadius:10,background:'rgba(255,179,64,0.08)',border:'1px solid rgba(255,179,64,0.20)'}}>
          ...
          <p style={{fontSize:11,color:'var(--text-mid)',margin:0}}>Ton plateau en musculation peut être dû à un manque de surcharge progressive. Essaie d'ajouter 2.5kg par semaine sur tes mouvements principaux.</p>
        </div>
      </Card>
    </div>
  )
...
export default function PerformancePage() {
  ...
  const TABS:[PerfTab,string,string,string][]=[
    ['profil',     '👤 Profil',      '#00c8e0','rgba(0,200,224,0.10)'],
    ['zones',      '⚡ Zones',       '#f97316','rgba(249,115,22,0.10)'],
    ['records',    '🏆 Records',     '#ffb340','rgba(255,179,64,0.10)'],
    ['progression','📈 Progression', '#22c55e','rgba(34,197,94,0.10)'],
  ]
  return (
    <div style={{padding:'24px 28px',maxWidth:'100%'}}>
      ...
      <div className="md:hidden" style={{display:'flex',gap:5,marginBottom:16,flexWrap:'wrap' as const}}>
        {TABS.map(([id,label,color,bg])=>(
          <button key={id} onClick={()=>setTab(id)}
            style={{flex:1,minWidth:80,padding:'8px 6px',borderRadius:10,border:'1px solid',cursor:'pointer',
              borderColor:tab===id?color:'var(--border)',background:tab===id?bg:'var(--bg-card)',
              color:tab===id?color:'var(--text-mid)',fontFamily:'Syne,sans-serif',fontSize:11,fontWeight:tab===id?700:400,
