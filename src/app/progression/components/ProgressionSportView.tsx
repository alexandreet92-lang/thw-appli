...
// ══════════════════════════════════════════════════════════════════
// ProgressionSportView — vue sport (réutilisable : rendue INLINE dans
// la section Progression de /activities pour ne jamais quitter le layout
// principal, et aussi via la route /progression/[sport]).
// Header + onglets familles. ⚠️ Métriques de progression bloquées
// (session_families + données structurées absentes) → état documenté.
// ══════════════════════════════════════════════════════════════════
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', background: 'var(--bg-card2)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 18px', fontSize: 13, color: 'var(--text-mid)', lineHeight: 1.55 }}>
        ...
        <div>
          ...
          <p style={{ margin: '6px 0 0', color: 'var(--text-dim)' }}>
            Le suivi nécessite la détection automatique des familles d&apos;effort
            (table <code>session_families</code>) et des données structurées (intervalles,
            segments Hyrox, exercices, 1RM/FTP/CSS) non encore disponibles. Aucune donnée
            n&apos;est inventée. Détails dans <code>PROMPT_PROGRESSION_COMPLETE.md</code>.
          </p>
        </div>
      </div>
    </div>
