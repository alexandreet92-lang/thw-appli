...
export default function RecoveryTimeline({ activities, recoveryScore }: Props) {
  ...
  const sport = (intense.sport_type ?? 'Séance').replace(/_/g, ' ')
  const W = 500, H = 80
  const pathLen = W * 1.5
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 20, padding: 24, boxShadow: 'var(--shadow-card)' }}>
      ...
      <h2 style={{ fontFamily: 'Syne,sans-serif', fontSize: 18, fontWeight: 700, margin: '0 0 14px' }}>Temps de récupération estimé</h2>

      {/* Info cards */}
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <div style={{ padding: '8px 14px', borderRadius: 10, background: 'var(--bg-card2)', border: '1px solid var(--border)' }}>
          <p style={{ fontSize: 9, color: 'var(--text-dim)', margin: '0 0 2px' }}>Dernière séance intense</p>
          <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)', margin: 0, textTransform: 'capitalize' }}>
            {sport} · TSS {tss} · il y a {fmtH(elapsedH)}
          </p>
        </div>
        {remainH > 0.5 ? (
          <div style={{ padding: '8px 14px', borderRadius: 10, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.3)' }}>
            <p style={{ fontSize: 9, color: 'var(--text-dim)', margin: '0 0 2px' }}>Récupération complète</p>
            <p style={{ fontSize: 12, fontWeight: 700, color: '#10B981', margin: 0 }}>dans {fmtH(remainH)}</p>
          </div>
        ) : (
          <div style={{ padding: '8px 14px', borderRadius: 10, background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.4)' }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: '#10B981', margin: 0 }}>✓ Récupéré</p>
          </div>
        )}
      </div>
      ...
      <div style={{ overflowX: 'auto' }}>
        <svg viewBox={`0 0 ${W} ${H+22}`} style={{ width: '100%', minWidth: 260, height: 'auto', display: 'block' }}>
          ...
          <text x={4} y={H*0.6-3}  fill="rgba(239,68,68,0.5)"    fontSize={7}>Non récupéré</text>
          <text x={4} y={H*0.25-3} fill="rgba(245,158,11,0.5)"   fontSize={7}>Partiel</text>
          <text x={4} y={8}        fill="rgba(16,185,129,0.6)"   fontSize={7}>Récupéré</text>
          {/* Fill under curve */}
          ...
          <text x={nowX} y={H+16} textAnchor="middle" fill="#7C3AED" fontSize={8} fontWeight={600}>Maintenant</text>
          {/* Full recovery marker */}
          {remainH > 0.5 && (
            <>
              <line x1={W} y1={0} x2={W} y2={H} stroke="#10B981" strokeWidth={1} opacity={0.5} />
              <text x={W - 4} y={H+16} textAnchor="end" fill="#10B981" fontSize={8} fontWeight={600}>Récupéré</text>
            </>
          )}
        </svg>
      </div>
