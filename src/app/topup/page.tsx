...
function PackCard({ pack, loading, onChoose }: { pack: Pack; loading: string | null; onChoose: (p: Pack) => void }) {
  ...
  const inner = (
    <div className="pack-card" onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      ...
      {feat && (
        <div style={{ position: 'absolute', top: 14, right: 14, display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 11px', borderRadius: 999, background: GRAD, color: '#fff', fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase', boxShadow: '0 2px 14px rgba(6,182,212,0.4)' }}>★ Recommandé</div>
      )}
      <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, color: 'var(--text-mid)', letterSpacing: '0.02em', marginBottom: 14 }}>{pack.name}</div>
      ...
      <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, lineHeight: 1.5, color: 'var(--text-mid)', flex: 1, margin: '6px 0 20px' }}>{pack.desc}</p>
      <button onClick={() => onChoose(pack)} disabled={!!loading} aria-label={`Choisir le pack ${pack.name}`}
        style={{
        ...
        {loading === pack.id ? <><span className="spinner" /> Redirection…</> : <>Choisir ce pack <ArrowRight size={15} /></>}
      </button>
    </div>
  )
...
function TopupInner() {
  ...
  async function choose(pack: Pack) {
    if (loading) return
    if (!session) { setError('Lien invalide — ouvre la page depuis le lien reçu par email.'); return }
    ...
    try {
      ...
      if (!res.ok || !json.checkout_url) throw new Error(json.error ?? 'Erreur')
  ...
  const planLabel = info ? `Plan ${info.plan.charAt(0).toUpperCase() + info.plan.slice(1)}` : 'Plan'
      ...
      <section style={{ paddingTop: 64, paddingBottom: 8 }}>
        <div className="topup-wrap" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          <div className="topup-reveal topup-eyebrow" style={{ marginBottom: 26 }}>
            ...
            Lien sécurisé · expire dans 24h
          </div>
          <div className="topup-reveal" style={{ marginBottom: 22 }}>
            <TokenGauge value={remaining} total={limit} label="Ton solde actuel" sub="quota mensuel" />
          </div>
          <div className="topup-reveal" style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '8px 16px', borderRadius: 999, border: '1px solid var(--border-mid)', background: 'var(--bg-card)' }}>
            ...
            {resetDays !== null && <>
              ...
              <span style={{ fontSize: 12.5, color: 'var(--text-mid)' }}>Réinitialisation dans {resetDays} jour{resetDays > 1 ? 's' : ''}</span>
            </>}
          </div>
        </div>
      </section>
        ...
        <div className="topup-wrap">
          <div className="topup-reveal" style={{ textAlign: 'center', maxWidth: 500, margin: '0 auto 44px' }}>
            <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: 'clamp(30px, 4.4vw, 44px)', fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.02, color: 'var(--text)', marginBottom: 16 }}>
              Recharge ton <span style={{ background: GRAD, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>carburant</span>
            </h1>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 15, lineHeight: 1.6, color: 'var(--text-mid)' }}>
              Des tokens supplémentaires, sans engagement. Ils s&apos;ajoutent à ton quota mensuel et n&apos;expirent jamais.
            </p>
      ...
      <section style={{ paddingTop: 96, paddingBottom: 8 }}>
        <div className="topup-wrap">
          <div className="topup-reveal" style={{ textAlign: 'center', marginBottom: 36 }}>
            <span className="topup-eyebrow" style={{ marginBottom: 14 }}>Repères</span>
            <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 'clamp(24px, 3.4vw, 34px)', fontWeight: 800, letterSpacing: '-0.035em', color: 'var(--text)', marginTop: 14 }}>À quoi correspondent ces tokens ?</h2>
          </div>
          <div className="usage-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, maxWidth: 780, margin: '0 auto' }}>
            {USAGES.map(({ Ic, title, amt, desc, c }, i) => (
              <div key={i} className="topup-reveal" style={{ position: 'relative', overflow: 'hidden', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 22px' }}>
                ...
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 15, fontWeight: 700, letterSpacing: '-0.01em', color: 'var(--text)', marginBottom: 5 }}>{title}</div>
                <div style={{ fontSize: 12.5, lineHeight: 1.5, color: 'var(--text-mid)' }}>{desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ paddingTop: 96, paddingBottom: 96 }}>
        <div className="topup-wrap">
          <div className="topup-reveal" style={{ textAlign: 'center', marginBottom: 44 }}>
            <span className="topup-eyebrow" style={{ marginBottom: 14 }}>3 étapes</span>
            <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 'clamp(24px, 3.4vw, 34px)', fontWeight: 800, letterSpacing: '-0.035em', color: 'var(--text)', marginTop: 14 }}>Comment ça marche</h2>
          </div>
          <div className="steps-grid" style={{ position: 'relative', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24, maxWidth: 820, margin: '0 auto' }}>
            ...
            {STEPS.map(({ Ic, title, desc }, i) => (
              <div key={i} className="topup-reveal" style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
                ...
                <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 16, fontWeight: 700, letterSpacing: '-0.01em', color: 'var(--text)', marginBottom: 7 }}>{title}</div>
                <div style={{ fontSize: 12.5, lineHeight: 1.55, color: 'var(--text-mid)', maxWidth: 220, margin: '0 auto' }}>{desc}</div>
              </div>
            ))}
          </div>
        </div>
