...
export function ReadOnlyActivityDetail({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  if (!row) {
    ...
    return createPortal(
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 14 }}>
