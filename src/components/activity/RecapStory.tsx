...
function periodBounds(period: 'week' | 'month', offset: number, ref: Date, t: TFn): Bounds {
  if (period === 'month') {
    ...
    <div style={{ padding: '64px 22px 24px', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {entries.length === 0 ? (
        <div style={{ margin: 'auto', textAlign: 'center', color: 'rgba(255,255,255,0.7)' }}>
          <div style={{ fontSize: 15, fontWeight: 700 }}>{t('lo.noDetailedPerf', { period: periodLabel })}</div>
          <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.5)', marginTop: 6, lineHeight: 1.5 }}>{t('lo.perfCalcHint')}</div>
        </div>
      ) : (
...
function MapCarousel({ acts, active, label }: { acts: RecapAct[]; active: boolean; label: string }) {
  ...
  const stats: { v: string; l: string }[] = [{ v: fmtH(a.moving_time_s ?? 0), l: t('lo.duration') }]
...
export function RecapStory({ period, activities, refDate, onClose }: {
  ...
  const cur = useMemo(() => computeStats(activities, periodBounds(period, 0, ref, t)), [activities, period, ref, t])
  const prev = useMemo(() => computeStats(activities, periodBounds(period, 1, ref, t)), [activities, period, ref, t])
  const trend = useMemo(() => Array.from({ length: N_TREND }, (_, i) => {
    const b = periodBounds(period, N_TREND - 1 - i, ref, t)
    return { b, st: computeStats(activities, b) }
  }), [activities, period, ref, t])
  const curBounds = useMemo(() => periodBounds(period, 0, ref, t), [period, ref, t])
  ...
  const pages = useMemo(() => {
    ...
    const per = t(period === 'week' ? 'lo.periodWeek' : 'lo.periodMonth')

    // 1 — Vue d'ensemble + tendance
    list.push({
      id: 'overview', bg: grad(accent), accent,
      share: { title: curBounds.label, subtitle: t(period === 'week' ? 'lo.recapWeekly' : 'lo.recapMonthly'), stats: [
        { label: t('lo.sessions'), value: String(cur.count) }, { label: t('lo.time'), value: fmtH(cur.time) },
        { label: t('lo.distance'), value: cur.dist > 0 ? `${Math.round(cur.dist / 1000)} km` : '—' }, { label: t('lo.loadSmShort'), value: String(Math.round(cur.sm)) },
      ] },
      render: (active) => {
        ...
        return (
          <div style={{ padding: '64px 26px 30px', height: '100%', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)' }}>
              {t(period === 'week' ? 'lo.recapWeekLong' : 'lo.recapMonthLong')}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '26px 20px', marginTop: 34 }}>
              <BigStat value={cur.count} label={t('lo.sessions')} accent={accent} active={active} delay={0} />
              <BigStat value={Math.round(cur.time / 3600)} label={t('lo.hours')} accent={accent} active={active} delay={90} />
              <BigStat value={Math.round(cur.dist / 1000)} label={t('lo.kilometers')} accent={accent} active={active} delay={180} />
              <BigStat value={Math.round(cur.sm)} label={t('lo.loadSm')} accent={accent} active={active} delay={270} />
            </div>
            {/* Mini-tendance */}
            <div style={{ marginTop: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.55)' }}>{t('lo.load')} · {t(period === 'week' ? 'lo.lastNWeeks' : 'lo.lastNMonths', { n: N_TREND })}</span>
    ...
    list.push({
      ...
      share: { title: `${t('lo.split')} — ${per}`, subtitle: t('lo.timePerSport'), stats: cur.bySport.slice(0, 4).map(s => ({ label: sportLabel(String(s.key)), value: fmtH(s.time) })) },
      render: (active) => {
        ...
        return (
          <div style={{ padding: '64px 26px 30px', height: '100%', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)' }}>{t('lo.split')}</div>
            <div style={{ fontSize: 22, fontWeight: 800, color: '#fff', marginTop: 4, marginBottom: 8 }}>{t('lo.whereTimeWent')}</div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '10px 0 18px' }}>
              <svg width="164" height="164" viewBox="0 0 140 140">
                ...
                <text x="70" y="84" textAnchor="middle" fontSize="9" fill="rgba(255,255,255,0.6)">{t('lo.total')}</text>
              </svg>
            </div>
            ...
            <div style={{ marginTop: 'auto', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              {[
                { l: t('lo.activeDays'), v: String(cur.activeDays) },
    ...
    cur.bySport.forEach((s) => {
      ...
      const shareStats: ShareStat[] = [{ label: t('lo.sessions'), value: String(s.count) }, { label: t('lo.time'), value: fmtH(s.time) }]
      if (s.dist > 0) shareStats.push({ label: t('lo.distance'), value: `${fmtKm(s.dist)} km` })
      shareStats.push({ label: t('lo.loadSmShort'), value: String(Math.round(s.sm)) })
      list.push({
        id: `sport-${s.key}`, bg: grad(c), accent: c,
        share: { title: `${sportLabel(String(s.key))} — ${per}`, subtitle: t(period === 'week' ? 'lo.summaryWeekly' : 'lo.summaryMonthly'), stats: shareStats },
        render: (active) => {
          const lines: { label: string; value: string; sub?: string }[] = [
            { label: t('lo.sessions'), value: String(s.count) },
            { label: t('lo.time'), value: fmtH(s.time) },
          ]
          if (s.dist > 0) lines.push({ label: t('lo.distance'), value: fmtKm(s.dist), sub: 'km' })
          if (s.elev > 0) lines.push({ label: t('lo.elevPlus'), value: `${Math.round(s.elev)}`, sub: 'm' })
          if (s.paceN > 0) lines.push({ label: t('lo.avgPace'), value: fmtPace(s.paceSum / s.paceN), sub: '/km' })
          else if (s.speedN > 0) lines.push({ label: t('lo.avgSpeed'), value: `${((s.speedSum / s.speedN) * 3.6).toFixed(1)}`, sub: 'km/h' })
          if (s.wattN > 0) lines.push({ label: t('lo.avgPower'), value: `${Math.round(s.wattSum / s.wattN)}`, sub: 'W' })
          if (s.hrN > 0) lines.push({ label: t('lo.avgHr'), value: `${Math.round(s.hrSum / s.hrN)}`, sub: 'bpm' })
          lines.push({ label: t('lo.loadSm'), value: String(Math.round(s.sm)) })
          return (
            <div style={{ padding: '64px 26px 30px', height: '100%', display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)' }}>{t('lo.sportSummary')}</div>
              ...
              {s.best && (
                <div style={{ marginTop: 'auto', background: 'rgba(255,255,255,0.1)', borderRadius: 14, padding: '13px 15px', display: 'flex', alignItems: 'center', gap: 11, opacity: active ? 1 : 0, transition: 'opacity .6s .5s' }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 700, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.best.title || t('lo.session')} · SM {Math.round(s.best.tss ?? 0)}</div>
                  </div>
                </div>
              )}
            </div>
    ...
    // Grosses séances (map + data, défilement auto) — plus longue par sport + plus intense
    {
      ...
      if (top.length > 0) {
        list.push({
          ...
          share: { title: `${t('lo.bigSessions')} — ${per}`, subtitle: t('lo.yourHighlights'), stats: top.slice(0, 4).map(a => ({ label: (a.title || t('lo.session')).slice(0, 16), value: fmtH(a.moving_time_s ?? 0) })) },
          render: (activePage) => <MapCarousel acts={top} active={activePage} label={t(period === 'week' ? 'lo.weekCap' : 'lo.monthCap')} />,
        })
      }
    }

    const perLabel = t(period === 'week' ? 'lo.ofTheWeek' : 'lo.ofTheMonth')
    ...
    if (cur.bySport.some(s => s.key === 'bike') || powerRec.length > 0) {
      list.push({
        ...
        share: { title: t('lo.recordsPower'), subtitle: `${t('lo.bike')} · ${perLabel}`, stats: powerRec.slice(0, 4).map(e => ({ label: `${e.label}${e.pr ? ' 🏆' : ''}`, value: `${e.periodValue} W` })) },
        render: (active) => <RecordsList title={t('lo.power')} entries={powerRec} accent="#5eead4" fmtVal={v => `${v} W`} active={active} periodLabel={perLabel} />,
      })
    }
    ...
    if (cur.bySport.some(s => s.key === 'run') || paceRec.length > 0) {
      list.push({
        ...
        share: { title: t('lo.recordsRun'), subtitle: `${t('lo.pace')} · ${perLabel}`, stats: paceRec.slice(0, 4).map(e => ({ label: `${e.label}${e.pr ? ' 🏆' : ''}`, value: fmtRecordTime(e.periodValue) })) },
        render: (active) => <RecordsList title={t('lo.runningTitle')} entries={paceRec} accent="#c4b5fd" fmtVal={fmtRecordTime} active={active} periodLabel={perLabel} />,
      })
    }
    ...
    list.push({
      ...
      share: { title: `${t('lo.competitions')} — ${per}`, subtitle: cur.racesList.length > 0 ? t(cur.racesList.length > 1 ? 'lo.raceCountPlural' : 'lo.raceCountSingular', { n: cur.racesList.length }) : t('lo.noRace'), stats: cur.racesList.slice(0, 4).map(r => ({ label: r.title || t('lo.raceFallback'), value: r.distance_m ? `${fmtKm(r.distance_m)} km` : fmtH(r.moving_time_s ?? 0) })) },
      render: (active) => (
        <div style={{ padding: '64px 26px 30px', height: '100%', display: 'flex', flexDirection: 'column' }}>
          <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)' }}>{t('lo.competitions')}</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: '#fff', marginTop: 4, marginBottom: 22 }}>{t(period === 'week' ? 'lo.yourBibsWeek' : 'lo.yourBibsMonth')}</div>
          {cur.racesList.length === 0 ? (
            <div style={{ margin: 'auto', textAlign: 'center', color: 'rgba(255,255,255,0.75)' }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 700, marginTop: 12 }}>{t(period === 'week' ? 'lo.noCompetitionWeek' : 'lo.noCompetitionMonth')}</div>
              <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', marginTop: 4 }}>{t('lo.markAsRaceHint')}</div>
            </div>
          ) : (
    ...
    list.push({
      ...
      share: { title: `${t('lo.highlights')} — ${per}`, subtitle: t('lo.yourMoments'), stats: [
        ...(cur.top ? [{ label: t('lo.mostIntense'), value: `SM ${Math.round(cur.top.tss ?? 0)}` }] : []),
        ...(cur.longest ? [{ label: t('lo.longest'), value: fmtH(cur.longest.moving_time_s ?? 0) }] : []),
        ...(cur.elev > 0 ? [{ label: t('lo.elevCumul'), value: `${Math.round(cur.elev)} m` }] : []),
        ...(cur.calories > 0 ? [{ label: t('lo.energy'), value: `${Math.round(cur.calories)} kcal` }] : []),
      ] },
      render: (active) => {
        const cards: { icon: ReactNode; label: string; value: string; sub: string }[] = []
        if (cur.top) cards.push({ icon: <IconTrophy size={22} color="#fff" />, label: t('lo.mostIntenseSession'), value: `SM ${Math.round(cur.top.tss ?? 0)}`, sub: cur.top.title || sportLabel(String(sportKeyFromType(cur.top.sport_type) ?? cur.top.sport_type)) })
        if (cur.longest) cards.push({ icon: <IconClock size={22} color="#fff" />, label: t('lo.longestSession'), value: fmtH(cur.longest.moving_time_s ?? 0), sub: cur.longest.title || '' })
        if (cur.elev > 0) cards.push({ icon: <IconMountain size={22} color="#fff" />, label: t('lo.elevCumulTitle'), value: `${Math.round(cur.elev)} m`, sub: t('lo.climbing') })
        if (cur.calories > 0) cards.push({ icon: <IconFlame size={22} color="#fff" />, label: t('lo.energySpent'), value: `${Math.round(cur.calories)} kcal`, sub: `${cur.races > 0 ? t(cur.races > 1 ? 'lo.raceCountPlural' : 'lo.raceCountSingular', { n: cur.races }) : t('lo.burned')}` })
        return (
          <div style={{ padding: '64px 26px 30px', height: '100%', display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)' }}>{t('lo.highlights')}</div>
            ...
            <button onClick={() => void shareCard({
              title: t(period === 'week' ? 'lo.myWeekTitle' : 'lo.myMonthTitle', { label: curBounds.label }),
              subtitle: t(period === 'week' ? 'lo.recapWeekly' : 'lo.recapMonthly'), accent,
              stats: [
                { label: t('lo.sessions'), value: String(cur.count) },
                { label: t('lo.time'), value: fmtH(cur.time) },
    ...
    return list
  }, [cur, prev, trend, accent, period, curBounds, powerRec, paceRec, recYear, t])
  ...
  return portal(
    <div style={overlay} role="dialog" aria-modal>
      ...
      >
        ...
        <button aria-label={t('lo.previous')} onPointerDown={stop} onClick={() => go(idx - 1)} style={{ position: 'absolute', left: 0, top: 90, bottom: 70, width: '25%', background: 'transparent', border: 'none', cursor: idx > 0 ? 'pointer' : 'default' }} />
