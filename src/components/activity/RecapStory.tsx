...
import { IconX, IconChevronLeft, IconChevronRight, IconShare2, IconTrophy, IconFlame, IconMountain, IconClock, IconBolt } from '@tabler/icons-react'
...
interface SportStat { key: SportKey | string; time: number; dist: number; count: number; sm: number; elev: number; hrSum: number; hrN: number; speedSum: number; speedN: number; paceSum: number; paceN: number; wattSum: number; wattN: number; best: RecapAct | null }
...
function computeStats(acts: RecapAct[], b: Bounds): PeriodStat {
  ...
  for (const a of inRange) {
    ...
    if (!s) { s = { key: k, time: 0, dist: 0, count: 0, sm: 0, elev: 0, hrSum: 0, hrN: 0, speedSum: 0, speedN: 0, paceSum: 0, paceN: 0, wattSum: 0, wattN: 0, best: null }; sportMap.set(k, s) }
    ...
    if (a.avg_pace_s_km) { s.paceSum += a.avg_pace_s_km; s.paceN++ }
    if (a.avg_watts) { s.wattSum += a.avg_watts; s.wattN++ }
...
export function RecapStory({ period, activities, refDate, onClose }: {
  ...
  const pages = useMemo(() => {
    const list: { id: string; bg: string; render: (active: boolean) => ReactNode }[] = []
    const grad = (c: string) => `linear-gradient(160deg, ${c} 0%, #0a0a0f 125%)`

    // 1 — Vue d'ensemble + tendance
    list.push({
      id: 'overview', bg: grad(accent),
      render: (active) => {
  ...
  return (
    <div style={overlay} role="dialog" aria-modal>
      <div
        style={{ position: 'relative', width: 'min(440px, 100%)', height: '100%', overflow: 'hidden', background: '#0a0a0f', boxShadow: '0 0 60px rgba(0,0,0,0.5)' }}
      ...
      >
        ...
        <div style={{ position: 'absolute', top: 'calc(10px + env(safe-area-inset-top))', left: 12, right: 12, zIndex: 5, display: 'flex', gap: 5 }}>
          {pages.map((_, i) => (
        ...
        {/* Fermer */}
        <button onClick={onClose} aria-label="Fermer" style={{ ...closeBtn, top: 'calc(24px + env(safe-area-inset-top))' }}><IconX size={20} color="#fff" /></button>
        ...
        <button aria-label="Précédent" onClick={() => go(idx - 1)} style={{ position: 'absolute', left: 0, top: 60, bottom: 90, width: '30%', background: 'transparent', border: 'none', cursor: idx > 0 ? 'pointer' : 'default' }} />
        <button aria-label="Suivant" onClick={() => go(idx + 1)} style={{ position: 'absolute', right: 0, top: 60, bottom: 90, width: '30%', background: 'transparent', border: 'none', cursor: 'pointer' }} />

        {/* Flèches desktop */}
        {idx > 0 && <button onClick={() => go(idx - 1)} aria-label="Précédent" style={{ ...navArrow, left: 8 }}><IconChevronLeft size={22} color="#fff" /></button>}
        {idx < nPages - 1 && <button onClick={() => go(idx + 1)} aria-label="Suivant" style={{ ...navArrow, right: 8 }}><IconChevronRight size={22} color="#fff" /></button>}
      </div>
    </div>
  )
}
