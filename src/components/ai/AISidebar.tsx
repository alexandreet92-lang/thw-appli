...
const AGENTS: { id: THWModel; name: string }[] = [
...
function ConvItem({ c, activeId, onSelect, onDelete, onPin }: {
  ...
  return (
    <div className="aip-hist-item" style={{ position: 'relative', marginBottom: 1 }}>
      <button onClick={() => onSelect(c)} style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: 'none', background: isActive ? 'var(--ai-accent-dim)' : 'transparent', cursor: 'pointer', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 1, transition: 'background 0.1s' }}
        onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLButtonElement).style.background = 'var(--ai-bg2)' }}
        onMouseLeave={e => { if (!isActive) (e.currentTarget as HTMLButtonElement).style.background = 'transparent' }}>
        <span style={{ fontSize: 12, fontWeight: isActive ? 600 : 400, color: isActive ? 'var(--ai-text)' : 'var(--ai-mid)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 170, fontFamily: 'DM Sans,sans-serif' }}>
          {c.isPinned && <span style={{ fontSize: 8, marginRight: 4, color: 'var(--ai-accent)' }}>*</span>}
          {c.title}
        </span>
        <span style={{ fontSize: 9, color: 'var(--ai-dim)', fontFamily: 'DM Mono,monospace' }}>{fmt(c.updatedAt)}</span>
      </button>
      <button className="aip-hist-dots" onClick={e => { e.stopPropagation(); setMenu(m => !m) }} style={{ position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)', width: 20, height: 20, border: 'none', background: 'none', cursor: 'pointer', color: 'var(--ai-dim)', opacity: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 4, padding: 0 }}>
      ...
      {menu && (
        <div ref={ref} style={{ position: 'absolute', right: 6, top: 'calc(100% - 4px)', zIndex: 50, background: 'var(--ai-bg)', border: '1px solid var(--ai-border)', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,0.2)', minWidth: 140, overflow: 'hidden' }}>
          {[
            ...
            <button key={it.label} onClick={it.action} style={{ display: 'block', width: '100%', padding: '8px 12px', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 12, fontFamily: 'DM Sans,sans-serif', color: (it as { danger?: boolean }).danger ? '#EF4444' : 'var(--ai-text)' }}>
              {it.label}
            </button>
          ))}
        </div>
...
export default function AISidebar({ convs, activeId, model, onSelect, onDelete, onNew, onPin, onModelChange, onClose, persistent = false }: Props) {
  ...
  const panel = (
    <div style={{ width: persistent ? 220 : 260, height: '100%', background: 'var(--ai-bg)', borderRight: '1px solid var(--ai-border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ borderTop: '1px solid var(--ai-border)', padding: '10px 10px 12px', flexShrink: 0 }}>
        <p style={{ fontSize: 9, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--ai-dim)', margin: '0 0 8px 2px' }}>Agent</p>
        <div style={{ display: 'flex', gap: 6 }}>
          {AGENTS.map(a => (
            <button key={a.id} onClick={() => onModelChange(a.id)} style={{ flex: 1, padding: '6px 4px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 600, fontFamily: 'DM Sans,sans-serif', transition: 'all 0.15s', background: model === a.id ? 'linear-gradient(135deg, #00c8e0, #3B82F6)' : 'var(--ai-bg2)', color: model === a.id ? '#fff' : 'var(--ai-mid)' }}>
              {a.name}
            </button>
          ))}
        </div>
