'use client'
// ══════════════════════════════════════════════════════════════════
// CoachSettingsSheet — réglages coach en SUR-PAGE coulissante (bas → haut),
// par-dessus la page courante. Se replie haut → bas (drag de la poignée ou
// tap sur le fond). Réutilise CoachSettingsContent — miroir exact de
// ProfileSheet. Mobile uniquement — monté globalement dans MobileShell,
// ouvert par l'event `thw:open-coach-settings`.
// ══════════════════════════════════════════════════════════════════
import { Suspense, useEffect, useRef, useState } from 'react'
import { CoachSettingsContent } from '@/app/coach/settings/page'
...
export function CoachSettingsSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="md:hidden" style={{ position: 'fixed', inset: 0, zIndex: 12000 }}>
      <style>{`
        @keyframes coachSheetIn { from { transform: translateY(100%) } to { transform: translateY(0) } }
        @keyframes coachSheetOut { from { transform: translateY(0) } to { transform: translateY(100%) } }
        @keyframes coachScrimIn { from { opacity: 0 } to { opacity: 1 } }
