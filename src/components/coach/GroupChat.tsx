    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 12000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, fontFamily: 'var(--font-body)' }}>
      ...
      <div style={{ position: 'relative', width: 'min(460px, 100%)', maxHeight: '82vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)', borderRadius: 20, overflow: 'hidden', boxShadow: '0 24px 70px rgba(0,0,0,0.4)' }}>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '0 18px' }}>
          ...
          {pool.length === 0 ? <div style={{ fontSize: 12.5, color: 'var(--text-dim)', padding: '10px 0' }}>{t('w2d.noOneToAddYet')}</div> : pool.map(p => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 5, background: 'var(--bg)', display: 'flex', flexDirection: 'column', animation: 'thwDDin 0.18s ease' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 16px 24px' }}>
        ...
        <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-dim)', margin: '14px 0 2px' }}>{t(members.length > 1 ? 'w2d.membersCountPlural' : 'w2d.membersCountSingular', { n: members.length })}</div>
        {members.map(m => (
          <div key={m.userId} style={row}>
            <Avatar url={m.avatar} name={m.name} size={34} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text)' }}>{m.name}{m.userId === me ? t('w2d.you') : ''}</div>
        ...
        {isAdmin && (
          adding ? (
            <div style={{ marginTop: 8, borderTop: '1px solid var(--border)', paddingTop: 10 }}>
              <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-dim)', margin: '0 0 4px' }}>{t('w2d.add')}</div>
              {pool.length === 0 ? <div style={{ fontSize: 12.5, color: 'var(--text-dim)', padding: '8px 0' }}>{t('w2d.noOneToAdd')}</div> : pool.map(p => (
