...
export function AthleteCoachCard() {
      ...
      if (links.length) {
        try {
          ...
          ;(data ?? []).forEach((p: { id: string; full_name: string | null; first_name: string | null; avatar_url: string | null }) =>
            names.set(p.id, { name: p.full_name || p.first_name || t('w2d.myCoach'), avatar: p.avatar_url ?? null }))
        } catch { /* profil coach non lisible → repli */ }
      }
      setCoaches(links.map(l => ({ ...l, name: names.get(l.coachId)?.name ?? t('w2d.myCoach'), avatar: names.get(l.coachId)?.avatar ?? null })))
  ...
  const add = async () => {
    ...
    try { await acceptInvite(code); setCode(''); setResetKey(k => k + 1); setAdding(false); setMsg({ text: t('w2d.coachAdded'), ok: true }); await reload() }
    catch (e) { setMsg({ text: e instanceof Error ? e.message : t('w2d.invalidCode'), ok: false }) }
    finally { setBusy(false) }
  }
  const remove = async (linkId: string) => {
    if (!confirm(t('w2d.confirmBreakLink'))) return
          ...
          {coaches.map(c => (
            <div key={c.linkId} style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
              <div style={{ position: 'relative', flexShrink: 0 }}>
                ...
                <span title={t('w2d.connected')} style={{ position: 'absolute', right: -1, bottom: -1, width: 14, height: 14, borderRadius: '50%', background: '#22c55e', border: '2.5px solid var(--bg-card)' }} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
