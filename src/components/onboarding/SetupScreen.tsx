  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(160deg,#0A0A0F 0%,#0F1A2E 100%)', display: 'flex', flexDirection: 'column', animation: 'ob-slide-in 320ms cubic-bezier(0.16,1,0.3,1)', paddingTop: 'env(safe-area-inset-top)' }}>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        <h2 style={{ fontSize: 28, fontWeight: 800, color: '#fff', margin: '0 0 6px', fontFamily: 'Syne, sans-serif' }}>Configurons ton profil</h2>
        <p style={{ fontSize: 14, color: dim, margin: 0, fontFamily: 'DM Sans, sans-serif' }}>Tu pourras tout modifier plus tard dans les réglages.</p>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 20px' }}>
        <NumRow label="FTP" desc="Puissance seuil fonctionnel" value={ftp} onChange={setFtp} min={50} max={600} step={5} unit="w" />
        <NumRow label="FC max" desc="Fréquence cardiaque maximale" value={maxHr} onChange={setMaxHr} min={100} max={220} step={1} unit="bpm" />
