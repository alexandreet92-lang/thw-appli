...
export function SessionEditor({ mode, session, dayIndex, plan, onClose, onSave, onDelete, onValidate, onAutoSave, onDuplicate, onCreateBrick, openWithFavorites, initialSport, reserveMode }: {
  ...
  useEffect(() => {
    ...
    const sameSport = session?.sport === 'gym' || session?.sport === 'hyrox'
    if (blks.length > 0 && sameSport) {
      const parsed = blocksToExercises(blks, sport === 'hyrox' ? 'hyrox' : 'gym')
      setExercises(parsed.exercises); setMCircuits(parsed.circuits); setMMap(parsed.map)
    } else {
