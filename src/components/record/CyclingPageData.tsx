...
function getLiveValue(fieldId: string, p: Props): string {
  ...
  switch (fieldId) {
    ...
    case 'avg_speed':        return avgSpeed.toFixed(1)
    case 'max_speed':        return p.speedKmh.toFixed(1)
    case 'lap_speed':        return p.currentLapSec > 0
      ? ((p.currentLapDistanceM / p.currentLapSec) * 3.6).toFixed(1)
      : '0.0'
    case 'elevation_gain':   return Math.round(p.elevationGainM).toString()
    case 'altitude':         return Math.round(p.altitudeM).toString()
    case 'gradient':         return '0.0'
