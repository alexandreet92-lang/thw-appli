...
const SECTION_ICONS: Record<string, ReactElement> = {
  ...
  postride:   (<svg width="18" height="18" viewBox="0 0 18 18" fill="none"><path d="M9 1l2 6h6l-5 4 2 6-5-4-5 4 2-6-5-4h6z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/></svg>),
}
...
function MTBSettingsInner({ open, onClose, isDark, settings, updateSetting: updateSetting_prop }: Props) {
  ...
  const addNewPage = () => {
    const newPage: DataPage = { id:`mtb_page_${Date.now()}`, name:`Page ${pages.length+1}`, type:'data', fields:['speed','duration','distance'] }
      ...
      {(pages as DataPage[]).map((page, idx) => (
        <div key={page.id} style={{ position:'relative' }}>
          <div style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 16px', borderBottom:`1px solid ${t.separator}` }}>
            ...
            <div style={{ width:28, height:28, borderRadius:'50%', flexShrink:0, background:`rgba(249,115,22,0.15)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, color:ACCENT }}>{idx+1}</div>
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <p style={{ fontSize:11, color:t.dim, margin:'2px 0 0', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                {page.type==='map' ? tr('record.mtbMapPrefix') : ''}{page.fields.map((id: string) => { const f = mtbFieldById(id); return f?.labelKey ? tr(f.labelKey) : f?.label }).filter(Boolean).join(' · ')}
              </p>
            </div>
    ...
    <SettingsSection title={tr('record.mtbSensorsSectionTitle')} theme={t}>
      ...
      {[
        ...
        <div key={s.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 16px', borderBottom: i < arr.length-1 ? `1px solid ${t.separator}` : 'none' }}>
          <div style={{ flex:1 }}>
            <p style={{ fontSize:15, color:t.text, margin:0 }}>{s.label}</p>
  ...
  const renderSectionContent = (id: string) => {
    ...
    if (id==='sensors')    return renderSensors()
    return <CyclingSettingsParams settings={settings as unknown as CyclingSettings} updateSetting={updateSetting} theme={t} section={id} />
  }
