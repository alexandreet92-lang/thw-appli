...
export default function PadelForm({ onClose }: Props) {
  ...
  if (saved) {
    ...
    const resultLabels: Record<Result, string> = { win: 'Victoire', loss: 'Défaite', draw: 'Nul' }
    ...
    return (
      <div style={{ position: 'fixed', inset: 0, zIndex: 10004, background: 'var(--bg)', color: 'var(--text)', display: 'flex', flexDirection: 'column', fontFamily: 'DM Sans, sans-serif', paddingTop: 'env(safe-area-inset-top)', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
        ...
        <p style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Séance enregistrée</p>
        ...
        <div style={{ textAlign: 'center', color: 'var(--text-dim)', fontSize: 14, lineHeight: 2 }}>
          <p style={{ margin: 0 }}>Durée : {fmtDur}</p>
          {saved.opponent && <p style={{ margin: 0 }}>Adversaire : {saved.opponent}</p>}
          <p style={{ margin: 0 }}>Surface : {PADEL_SURFACES.find(s => s.id === saved.surface)?.label}</p>
          <p style={{ margin: 0 }}>RPE : {saved.rpe}/10</p>
        </div>
        <button onClick={onClose} style={{ padding: '14px 40px', borderRadius: 16, background: 'linear-gradient(135deg,#06B6D4,#2563EB)', border: 'none', color: '#FFF', fontSize: 16, fontWeight: 600, cursor: 'pointer' }}>Terminer</button>
      </div>
    )
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 10004, background: 'var(--bg)', color: 'var(--text)', display: 'flex', flexDirection: 'column', fontFamily: 'DM Sans, sans-serif', paddingTop: 'env(safe-area-inset-top)' }}>
      <div style={{ height: 52, flexShrink: 0, display: 'flex', alignItems: 'center', padding: '0 16px', borderBottom: '1px solid var(--border)', position: 'relative' }}>
        ...
        <span style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', fontSize: 15, fontWeight: 600 }}>Nouvelle séance</span>
        <button onClick={handleSave} disabled={saving} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#06B6D4', fontSize: 15, fontWeight: 600, cursor: saving ? 'default' : 'pointer', opacity: saving ? 0.5 : 1 }}>{saving ? '…' : 'Enregistrer'}</button>
      </div>

      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 20px', paddingBottom: 120 }}>
        <div style={{ marginBottom: 24 }}><label style={LABEL}>Sport</label><Chips items={PADEL_SPORTS} value={sport} onChange={setSport} /></div>
        <div style={{ marginBottom: 24 }}>
          <label style={LABEL}>Adversaire</label>
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            {(['Moi', 'Double'] as const).map(m => (
              <button key={m} onClick={() => setIsDouble(m === 'Double')} style={{ padding: '8px 20px', borderRadius: 20, border: 'none', background: (m === 'Double') === isDouble ? '#06B6D4' : 'var(--bg-card2)', color: (m === 'Double') === isDouble ? '#FFF' : 'var(--text)', fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>{m}</button>
            ))}
          </div>
          <input value={opponent} onChange={e => setOpponent(e.target.value)} placeholder="Nom de l'adversaire" style={{ ...INPUT, marginBottom: isDouble ? 8 : 0 }} />
          {isDouble && <input value={partner} onChange={e => setPartner(e.target.value)} placeholder="Partenaire" style={INPUT} />}
        </div>
        <div style={{ marginBottom: 24 }}>
          <label style={LABEL}>Lieu</label>
          <input value={location} onChange={e => setLocation(e.target.value)} placeholder="Nom du club ou terrain" style={{ ...INPUT, marginBottom: 10 }} />
          <Chips items={PADEL_SURFACES} value={surface} onChange={setSurface} />
        </div>
        <div style={{ marginBottom: 24 }}>
          <label style={LABEL}>Résultat</label>
        ...
        <div style={{ marginBottom: 24 }}><label style={LABEL}>Score par sets</label><MatchScoreInput sets={sets} onChange={setSets} isDark={true} /></div>
        <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ marginBottom: 24 }}><label style={LABEL}>Ressenti (RPE)</label><RPESlider value={rpe} onChange={setRpe} isDark={true} /></div>
        <div style={{ marginBottom: 12 }}><label style={LABEL}>Commentaire</label><textarea value={comment} onChange={e => setComment(e.target.value)} rows={4} placeholder="Décris ta séance…" style={{ ...INPUT, resize: 'none' }} /></div>
      </div>
