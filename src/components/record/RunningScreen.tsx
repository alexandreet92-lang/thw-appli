...
interface SessionSnap {
  ...
  distM: number; elevM: number; avgSpeedKmh: number; maxSpeedKmh: number
...
export default function RunningScreen({ onExit, onFinished, route }: Props) {
  ...
  const handleOpenSaveForm = () => {
    ...
    snapRef.current = { startedAtISO: startedAt ? new Date(startedAt).toISOString() : endedAt.toISOString(), endedAtISO: endedAt.toISOString(), durationSec, distM, elevM, avgSpeedKmh, maxSpeedKmh: parseFloat(gps.maxSpeed.toFixed(1)), calories: Math.round((durationSec / 3600) * 500), gpsPts: [...gps.points], lapsSnap: [...laps] }
    stopWatching()
    setPhase('paused')
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, backgroundColor: bg, color: text, display: 'flex', flexDirection: 'column', width: '100vw', height: '100dvh', paddingTop: 'env(safe-area-inset-top)' }}>
      ...
      {navOpen && (
        <RouteNavScreen route={route ?? null} sport="running" showWatts={false} hr={null} onClose={() => setNavOpen(false)} />
      )}
