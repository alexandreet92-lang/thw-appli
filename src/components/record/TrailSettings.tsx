...
function TrailSettingsInner({ open, onClose, isDark, settings, updateSetting: updateSetting_prop }: Props) {
  ...
  const sectionDesc = (id: string) => tr(`record.trailSectionDesc_${id}`)
  ...
  const renderPagesSection = () => (
    <div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px 8px' }}>
        ...
        <button onClick={addNewPage} style={{ fontSize:12, color:'#F59E0B', background:'none', border:'none', cursor:'pointer', fontWeight:600 }}>{tr('record.settingsAdd')}</button>
      </div>
