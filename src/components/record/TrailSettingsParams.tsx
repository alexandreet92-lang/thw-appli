...
export default function TrailSettingsParams({ settings, updateSetting, theme, section }: Props) {
  ...
  const renderAlerts = () => (
    <SettingsSection title="NOTIFICATIONS & ALERTES" theme={theme}>
      ...
      <SettingsRow theme={theme} label="Alerte zone FC" disabled={!sensor.hr}
        right={<Toggle theme={theme} value={al.hrZone} onChange={v => updateSetting('alerts.hrZone', v)} disabled={!sensor.hr} />} />
      <SettingsRow theme={theme} label="Seuil FC max" disabled={!sensor.hr}
      ...
      <SettingsSectionSubtitle label="Alerte terrain" theme={theme} />
      <SettingsRow theme={theme} label="Alerte pente dangereuse" description="Notifie si la pente dépasse le seuil"
  ...
  const renderSensors = () => (
    <SettingsSection title="CAPTEURS" theme={theme}>
      <div style={{ padding:'10px 16px 14px', background:'rgba(245,158,11,0.06)', borderBottom:`1px solid ${theme.separator}` }}>
        <p style={{ fontSize:12, color:'#F59E0B', margin:0, lineHeight:1.5 }}>La connexion Bluetooth sera disponible lors du lancement sur l&apos;App Store.</p>
      </div>
      {[
        ...
        <div key={s.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 16px', borderBottom: i < arr.length-1 ? `1px solid ${theme.separator}` : 'none' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <span style={{ fontSize:10, color:'#F59E0B', border:'1px solid rgba(245,158,11,0.4)', borderRadius:20, padding:'2px 8px' }}>Bientôt</span>
          </div>
        </div>
      ))}
    </SettingsSection>
  ...
  const renderAthlete = () => (
    <SettingsSection title="PROFIL ATHLÈTE" theme={theme}>
      ...
      <SettingsRow theme={theme} label="VMA" description="Vitesse maximale aérobie"
      ...
      <SettingsRow theme={theme} label="UTMB Index" description="Score de référence pour le trail"
        right={<NumberInput theme={theme} value={at.utmbIndex} min={0} max={1000} step={1} unit="" onChange={v => updateSetting('athlete.utmbIndex', v)} />} />
      <SettingsRow theme={theme} label="Vitesse ascensionnelle" description="Calculée automatiquement depuis tes sorties"
  ...
  const renderPostRun = () => (
    <SettingsSection title="APRÈS LA SÉANCE" theme={theme}>
      <SettingsRow theme={theme} label="Upload automatique Strava" disabled={!stravaConnected}
        right={<Toggle theme={theme} value={po.autoStrava} onChange={v => updateSetting('postRun.autoStrava', v)} disabled={!stravaConnected} />} />
      {stravaConnected
        ? <div style={{ padding:'4px 16px 8px', display:'flex', alignItems:'center', gap:6 }}><div style={{ width:6, height:6, borderRadius:'50%', background:'#10B981' }} /><span style={{ fontSize:12, color:'#10B981' }}>Strava connecté</span></div>
        : <p style={{ fontSize:12, color:'#8C8C8C', padding:'4px 16px 8px', margin:0 }}><a href="/connections" style={{ color:'#F59E0B', textDecoration:'none' }}>Connecte Strava</a> dans la page Connexions.</p>
      }
      <SettingsRow theme={theme} label="Résumé en fin de séance" last
