  ...
  return (
    <div className="card-enter card-enter-1" style={{ background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:20,padding:24,boxShadow:'var(--shadow-card)',marginBottom:16 }}>
      <div style={{ marginBottom:18 }}>
        ...
        <h2 style={{ fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:700,margin:'3px 0 0' }}>Charge d'entraînement</h2>
      </div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))',gap:10,marginBottom:20 }}>
        ...
        <div style={{ padding:'12px',borderRadius:12,background:'var(--bg-card2)',border:'1px solid var(--border)',textAlign:'center' as const }}>
          ...
          <p style={{ fontSize:9,color:'var(--text-dim)',margin:'4px 0 0',textTransform:'uppercase' as const,letterSpacing:'0.06em' }}>Volume</p>
          <p style={{ fontSize:9,margin:'2px 0 0',fontWeight:600,
            color:delta>=0?'#22c55e':'#ef4444' }}>
            {delta>=0?'+':''}{delta.toFixed(1)}h vs sem. préc.
          </p>
        </div>
      </div>
      ...
      {data.breakdown.length > 0 && (
        <div>
          <p style={{ fontSize:10,fontWeight:700,textTransform:'uppercase' as const,letterSpacing:'0.07em',color:'var(--text-dim)',margin:'0 0 10px' }}>Répartition</p>
