...
const CADENCES: { v: Cadence; k: string }[] = [{ v: 'basse', k: 'session.basse' }, { v: 'haute', k: 'session.haute' }]
const TERRAINS: { v: Terrain; k: string }[] = [{ v: 'plat', k: 'session.plat' }, { v: 'cote', k: 'session.cote' }]
      ...
      <AnimatePresence>
        ...
        {p.open && (
          <motion.div key="panel" initial={reduce ? { opacity: 0 } : { y: '100%' }} animate={reduce ? { opacity: 1 } : { y: 0 }}
            ...
            style={{ position: 'relative', background: 'var(--bg-card)', borderRadius: 'var(--r-lg) var(--r-lg) 0 0', maxHeight: 'calc(100dvh - 72px)', display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow)' }}>
