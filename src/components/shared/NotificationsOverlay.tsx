...
function timeAgo(iso: string, t: TFn): string {
  ...
  if (m < 1) return t('shared.justNow')
  if (m < 60) return t('shared.minAgo', { n: m })
  const h = Math.floor(m / 60)
  if (h < 24) return t('shared.hoursAgo', { n: h })
  const d = Math.floor(h / 24)
  return d === 1 ? t('shared.yesterday') : t('shared.daysAgo', { n: d })
}
  ...
  return createPortal(
    <div onClick={e => { if (e.target === e.currentTarget) onClose() }}
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 20, width: 'min(480px, 94vw)', maxHeight: '80vh', overflowY: 'auto', padding: '24px 26px', border: '1px solid var(--border)', boxShadow: '0 24px 60px rgba(0,0,0,.18)', transform: shown ? 'scale(1)' : 'scale(0.92)', transition: 'transform .3s cubic-bezier(.2,.8,.2,1)' }}>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '24px 0', color: 'var(--text-mid)', fontSize: 14 }}>{t('shared.loading')}</div>
        ) : notifs.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            <p style={{ margin: 0, fontFamily: FD, fontSize: 16, fontWeight: 500, color: 'var(--text)' }}>{t('shared.nothingNew')}</p>
            <p style={{ margin: '6px 0 0', fontSize: 13, color: 'var(--text-mid)', lineHeight: 1.6 }}>{t('shared.alertsHint')}</p>
          </div>
        ) : (
