...
export function UpgradeModalHost() {
  ...
  const perks = [
    'IA complète : messages, plans d’entraînement et de nutrition illimités selon l’offre',
    'Briefing quotidien et recherche web (Pro / Expert)',
    'Automatisations, Studio d’agents et mémoire long terme',
    'Historique étendu et synchronisation Strava sans plafond',
  ]
    ...
    <div onClick={() => setOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 12000, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: 'min(440px, 100%)', background: 'var(--bg-card)', borderRadius: 'var(--r-lg)', padding: 24, boxShadow: '0 24px 70px rgba(0,0,0,0.35)' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 600, color: 'var(--text)', margin: 0 }}>Passe à une offre</h2>
          <button onClick={() => setOpen(false)} aria-label="Fermer" style={{ border: 'none', background: 'transparent', color: 'var(--text-dim)', cursor: 'pointer', display: 'flex', padding: 2 }}>
        ...
        <p style={{ fontFamily: 'var(--font-body)', fontSize: 13.5, color: 'var(--text-mid)', margin: '8px 0 16px', lineHeight: 1.5 }}>
          {reason ?? "Tu as atteint une limite de ton offre actuelle."}
        </p>
