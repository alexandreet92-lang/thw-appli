...
export function useMTBSettings(onSaved?: () => void) {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        if (!user) return
        const { data } = await supabase.from('mtb_settings').select('settings').eq('user_id', user.id).maybeSingle()
        if (data?.settings) {
          const merged = { ...DEFAULT_MTB_SETTINGS, ...(data.settings as Partial<MTBSettings>) } as MTBSettings
          setSettings(merged); latestRef.current = merged
        } else {
          await supabase.from('mtb_settings').upsert({ user_id: user.id, settings: DEFAULT_MTB_SETTINGS }, { onConflict: 'user_id' })
        }
      } catch { /* table absent */ }
  ...
  const persistSettings = useCallback(async (next: MTBSettings) => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) return
      const { error } = await supabase.from('mtb_settings').upsert({ user_id: user.id, settings: next, updated_at: new Date().toISOString() }, { onConflict: 'user_id' })
