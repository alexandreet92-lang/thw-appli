// ══════════════════════════════════════════════════════════════════════════
// Moteur d'analyse de séance (partagé).
// Extrait de /api/analyze-training pour être réutilisable :
//   • par la route (déclenchement MANUEL depuis l'app) ;
//   • par le déclenchement AUTOMATIQUE après sync (Brique 1 — cron auto-analyze).
...
/** Niveau de détail de l'analyse choisi par l'utilisateur. */
export type AnalysisDetail = 'minimum' | 'advanced' | 'specialist'

export interface AnalyzeTrainingInput {
  activities: AnalyzeActivityInput[]
  zones: unknown
  planned: unknown
  recovery: unknown[]
  similar: unknown[]
...
const DETAIL_TOKENS: Record<AnalysisDetail, number> = { minimum: 3500, advanced: 8000, specialist: 12000 }
const DETAIL_TOKENS_COMPARE: Record<AnalysisDetail, number> = { minimum: 5000, advanced: 10000, specialist: 14000 }
const DETAIL_DIRECTIVE: Record<AnalysisDetail, string> = {
  minimum: "\n\nNIVEAU DE DÉTAIL DEMANDÉ : MINIMUM. Va droit au but : 'execution' en 1 paragraphe court, 1 seul conseil, pas de tableaux.",
  advanced: '',
  specialist: "\n\nNIVEAU DE DÉTAIL DEMANDÉ : SPÉCIALISTE. Analyse très technique et détaillée pour un athlète averti : physiologie fine, tableaux quand pertinent, jusqu'à 3 conseils pointus.",
}
...
function streamSummary(streams: StreamData): string {
  ...
  const lines: string[] = []
  const summarize = (arr: number[] | undefined, label: string, unit: string, fmt?: (v: number) => string) => {
    ...
    lines.push(`${label}: moy=${f(avg)} min=${f(min)} max=${f(max)} Q1_moy=${f(avgQ1)} Q4_moy=${f(avgQ4)} (${arr.length} pts) ${unit}`)
  }
...
function lapsText(laps: LapData[] | undefined): string {
  ...
  return laps
    .map((l, i) => {
      ...
      const pace = l.avg_speed_ms && l.avg_speed_ms > 0
...
/**
 * Construit les prompts + appelle le LLM + parse la sortie JSON.
 * Renvoie le rapport (objet JSON) ou lève une erreur.
 */
export async function runTrainingAnalysis(body: AnalyzeTrainingInput): Promise<unknown> {
  const { activities, zones, planned, recovery, similar, tssWeekBefore, aiRules } = body
  const detail: AnalysisDetail = body.detail ?? 'advanced'
...
Tu réponds UNIQUEMENT en JSON valide. Zéro texte avant ou après.${DETAIL_DIRECTIVE[detail]}`
...
Retourne ce JSON (mode: "${isComparison ? 'comparison' : 'single'}") :
{
  ...
  const maxTokens = (isComparison ? DETAIL_TOKENS_COMPARE : DETAIL_TOKENS)[detail]
