...
export interface RosterAthlete {
  ...
  sports: string[]
  level: string | null
  goal: string | null        // type d'objectif (profiles.primary_goal : performance, force…)
  gender: string | null      // profiles.gender (male / female / …)
  group: string | null
  note: string | null
  status: Forme
  ...
  tsb: number | null         // fraîcheur du jour (PMC) ; ≤ -20 = surcharge
  adhDone: number
  adhTotal: number
...
export async function getRoster(): Promise<RosterAthlete[]> {
  ...
  return rows.map(r => {
    ...
    // Forme + raison (priorité : blessé > inactif > surcharge > attention > ok).
