...
export function I18nProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (isLang(stored)) { setLangState(stored); document.documentElement.lang = stored; return }
    ...
    void (async () => {
      try {
        ...
        if (isLang(l)) { setLangState(l); document.documentElement.lang = l; try { localStorage.setItem(LS_KEY, l) } catch { /* ignore */ } }
      } catch { /* ignore */ }
    })()
  }, [])
