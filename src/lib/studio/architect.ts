  ...
  for (const an of plan.nodes.slice(0, 8)) {
    ...
    nodes.push({
      ...
      actionKey: an.kind === 'action'
        ? (['planning_save', 'planning_replace', 'calendar_race', 'nutrition_save', 'notify_report'].includes(String(an.actionKey)) ? an.actionKey : 'planning_save')
        : undefined,
    })
  }
...
- "action" : écrit dans l'app. "actionKey" ∈ "planning_save" (AJOUTE des séances au Planning) | "planning_replace" (REMPLACE les séances IA de la semaine — nettoie puis réécrit) | "calendar_race" (ajoute une course au Calendrier) | "nutrition_save" (enregistre un plan nutrition qui devient le plan actif) | "notify_report" (envoie le bilan en notification). Seulement si l'utilisateur veut vraiment écrire.
